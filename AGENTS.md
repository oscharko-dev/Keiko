...
| Added, renamed, or deleted anything under `tests/e2e/` | `npm run check:e2e-suite-wiring` — a suite no lane runs is not coverage (#2629), a spec no script can reach is not a suite, and a retained spec calling an unmounted route is a journey that cannot pass (#2955). Reads the built `API_ROUTES`, so run `npm run build:packages` (or `npm run typecheck`) first |
