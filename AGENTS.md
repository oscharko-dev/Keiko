...
| The ESLint toolchain (`eslint`, `@eslint/js`, a workspace's `eslint` range, `eslint.config.*`) | `npm run check:eslint-lane` and `npm run check:dependency-hygiene` — #2777: `npm ls` cannot see `@eslint/js` drifting a major behind `eslint` (its peer is optional), nor a second ESLint installed under a workspace, so both are gated here |
