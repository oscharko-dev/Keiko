...
(function () {
  ...
  /* KEIKO-0911 not applicable to this ICON map: theme-control.js's moon/sun renders at
     stroke-width="1.7" (matching the auto/high siblings in this map for visual parity),
     while LiftGlyph.svg() unconditionally emits stroke-width="1.6". Consolidating would
     change the rendered stroke thickness in the palette picker where all four icons
     appear together — the exact class the doNot warns against. The inline paths below
     also use a full <circle r="4"> for the sun's center, not LiftGlyph.G.sun's arc-with-
     gap — same geometry-change guardrail. Kept inline deliberately. */
  var ICON = {
  ...
  function applyTheme(t) {
    ...
    var l = t === "light";
    document.querySelectorAll("[data-theme-label]").forEach(function (e) {
      e.textContent = l ? "Light" : "Dark";
    });
  }
  function applyContrast(c) {
    if (c === "more") root.dataset.hc = "more";
  ...
  function seg(labelText, ariaLabel, opts) {
    ...
    function select(val, focus) {
      paint(val);
      opts.forEach(function (o) {
        if (o.val === val) o.onpick();
      });
      if (focus)
        buttons.forEach(function (b) {
          if (b.dataset.val === val) b.focus();
        });
    }
  ...
  function build() {
    ...
    var contrast = seg("Contrast", "Contrast", [
      {
        ...
        label: "Auto",
        icon: "auto",
        aria: "Contrast follows the operating system",
        onpick: function () {
          applyContrast("auto");
          reflectOS();
        },
    ...
    function reflectOS() {
      var highBtn = contrast.buttons.find(function (b) {
        return b.dataset.val === "more";
      });
