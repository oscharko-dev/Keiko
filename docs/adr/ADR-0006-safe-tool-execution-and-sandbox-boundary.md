...
| `npx` | unrestricted | Package runner; subcommands are dynamically resolved |
| `npm` | denylist: publish/unpublish/login/logout/adduser/token/version/deprecate/owner/access/star/profile | Allow run/test/ci/install; deny account- and registry-mutating operations |
...
`basename(executable)` against the table (`src/tools/sandbox.ts:95–106`), so a caller cannot bypass
the allowlist by passing an absolute path to an allowed executable. Unknown executables are denied
by default.

All spawns use `{ shell: false }` unconditionally (`src/tools/exec.ts:256`). Args are passed as an
array to `child_process.spawn`; no argument string is ever interpolated into a shell command.
...
without throwing for a validation failure; it throws only for a parse error. Checks in order:
...
3. **Path safety** — every target path is checked via `resolveWithinWorkspace` (rejects `..`, abs, NUL) and `isDenied` (always-on deny list from ADR-0005 D3: `.env*`, `*.pem`, `*.key`, `.git/**`, `node_modules/**`, etc.).
...
- **Why rejected**: shell execution is incompatible with fail-closed security in a regulated
  environment. The no-shell, deny-by-default allowlist provides a provably smaller attack surface
  because the set of possible subprocess spawns is the finite allowlist, not "any valid shell
  expression".
...
- **Cons**: any npm dependency violates ADR-0001's zero-runtime-dependency constraint. Invoking
  `git apply` as a subprocess introduces a different problem: `git apply` can execute hooks
  (`pre-applypatch`, `applypatch-msg`, `post-applypatch`) which are arbitrary scripts in the
  repository. This is a code-execution vulnerability if the repository is untrusted or if a
  malicious commit adds a hook. It also has different security properties (git apply writes
  directly, no rollback) and would need to be added to the command allowlist.
