...
Accepted; superseded in part by ADR-0043 (the D2 network-enforcement row — network egress can now be OS/container-enforced via `keiko-sandbox` for `network:"none"` runs, rather than being unconditionally documented-but-not-enforced as originally decided here).
