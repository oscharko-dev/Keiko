...
Superseded in part by ADR-0019 and ADR-0025 for module-layout/file-path details only — workflow code relocated from `src/workflows/unit-tests/**` and `src/cli/gen-tests.ts` to `packages/keiko-workflows/src/unit-tests/**` and `packages/keiko-cli/src/gen-tests.ts`. D2–D8 and D10–D12 remain the active design, still cited by decision letter throughout current source.
