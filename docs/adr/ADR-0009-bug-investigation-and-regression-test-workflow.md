...
Superseded by ADR-0019 / ADR-0025 for the physical module layout only — `src/workflows/bug-investigation/**` moved to `packages/keiko-workflows/src/bug-investigation/**`, and D12's shared descriptor moved again to `packages/keiko-contracts/src/workflow-descriptor.ts` (issue #158). All other decisions (D2–D11, D13–D16) remain the accurate, currently-implemented design.
