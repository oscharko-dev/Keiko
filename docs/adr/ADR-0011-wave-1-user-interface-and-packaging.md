...
Superseded by ADR-0019/ADR-0020/ADR-0021/ADR-0025 for packaging shape only (D3/D4/D6: the non-workspace nested `ui/`, zero-runtime-deps root, and `src/ui/**` paths were all reversed by the modular npm-workspace monorepo). D1/D2/D5/D7–D12 remain the live precedent for `packages/keiko-server` and `packages/keiko-ui`, actively cited by ADR-0023, ADR-0035, and ADR-0126.
...
| 2   | `GET /api/config`              | —                                                                                                         | `{ "config": SafeGatewayConfig \| null, "configPresent": boolean }`                                                                                                                               | Via `toSafeObject(config)`. **Never** `apiKey` or provider `baseUrl`. `null` when no config file resolved.                                                                                                                                                                                       |
...
| 12  | `GET /api/workspace`           | query: required `?dir`, optional `task`, optional positive integer `budget`                                | `{ "summary": WorkspaceSummary }`                                                                                                                                                                 | Workspace-layer summary endpoint for a registered local project. Builds the summary from the safe workspace primitives, includes a context pack when `task` or `budget` is provided, and redacts response bodies before serialization.                                                           |
...
- **Config display** goes through `toSafeObject(config)` → `SafeGatewayConfig` (strips `apiKey`
  and provider `baseUrl`) before serialization (route 2).
