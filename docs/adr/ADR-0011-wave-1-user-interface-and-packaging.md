...
| 5 | `POST /api/runs` | `{ "workflowId"?: string, "taskType"?: TaskType, "input": object, "modelId": string, "apply"?: boolean, "limits"?: object }` | `202` `{ "runId": string, "fingerprint": string }` | Exactly one of `workflowId`/`taskType`. `apply` **defaults `false`** (D8). Server starts the run, registers the streaming sink (D7), returns immediately. |
