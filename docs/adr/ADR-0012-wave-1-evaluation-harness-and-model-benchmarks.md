...
Superseded in part by ADR-0019 (Modular Package Architecture) for D1 and the Alternative-4 rejection — the harness now lives in `packages/keiko-evaluations/` with shared types in `packages/keiko-contracts/src/evaluations.ts`; the audit-completeness integration point (D9) is `packages/keiko-evidence`, not `src/ui/evidence.ts`. All other decisions (D2–D13) remain the live design rationale cited throughout current code.
