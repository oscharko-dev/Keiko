...
Superseded by ADR-0019 for paths only — `src/ui/store/*`, `src/audit/`, `ui/lib/`, and `ui/app/components/shell/*` now live under `packages/keiko-server/src/` and `packages/keiko-ui/src/`. The PATCH-not-append, polling-not-SSE, and evidence-fallback decisions themselves are unchanged and still enforced.
