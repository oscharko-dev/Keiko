...
Issue #287 adds rule `direction-10a` to the architecture gate and registers it in the negative-fixture expectations (`scripts/arch-check-negative.mjs` `EXPECTED_DEPCRUISER_RULE_COUNTS`).
...
| Repository detection, safe file reads, context packs          | `keiko-workspace`                                                                 | reuse-as-is                                       | No new code required; existing `detectWorkspace()`, `readWorkspaceFile()`, `buildContextPack()`.                                              | #278         |
...
| Provider credential config, model capability probe            | `keiko-model-gateway` + `keiko-server`                                            | reuse-as-is                                       | Existing `GatewaySetup`, `listConfiguredCapabilities()`, BFF route `/api/gateway/setup`.                                                      | #279         |
