...
The existing `WindowsRegistry.ts` already lists the typed window types enumerated in the WindowType union and exposes `registerWindowRender(type, render)` as the extension contract bound in `widgets/index.tsx`. The [product boundary issue](../workspace/518-product-boundaries.md) defined per-object lifecycle, trust boundary, authority requirement, and persistence expectation. This ADR formalizes those fields as a sidecar metadata table (`WIN_META`) and adds a metadata validator.
