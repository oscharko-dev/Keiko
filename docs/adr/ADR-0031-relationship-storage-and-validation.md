...
The v28 migration (Issue #3400) is the first migration to extend the `CHECK` constraint added at V5: SQLite cannot widen a `CHECK` with `ALTER TABLE`, so the migration rebuilds `relationships` under the widened kind lists (adding `git-change` to both the source-kind and target-kind enumerations) and copies every existing row across unchanged. Because `relationship_lifecycle_history.relationship_id REFERENCES relationships(id) ON DELETE CASCADE` is a live foreign key, dropping and recreating `relationships` first would cascade-delete every history row before the rebuilt table exists to receive them — so the migration also rebuilds `relationship_lifecycle_history` itself: create `relationship_lifecycle_history_v28` pointed at the new `relationships`, `INSERT` every existing row into it, `DROP` the old `relationship_lifecycle_history`, then `RENAME` the `_v28` table into its place (never `DROP`-then-recreate the child first). The table object is rebuilt; every row's data is preserved unchanged. This rename-then-rebuild-child technique is the same additive-migration shape the "Future schema evolution" note below already sanctions; v28 is its first real occurrence.
