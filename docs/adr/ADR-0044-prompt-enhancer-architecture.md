...
| Retrieval plan + source policy emission                                                                                                                                  | `keiko-contracts` shape + `keiko-local-knowledge` additive describe API; binding in `keiko-server` | reuse `runLocalKnowledgeRetrieval`, `buildComposedRetrievalScope`, `runHybridGroundedAsk`; **emit a plan, never a second retrieval engine**                                       | #1311        |
...
| BFF `/api/prompt-enhancer/*` routes                                                                                                                                      | `keiko-server`                                                                                     | reuse `API_ROUTES`, `UiHandlerDeps`, `readBody`/`parseBody`, `mappedGatewayError`, SSE helpers                                                                                    | #1314        |
...
- The retrieval-plan boundary means the enhancer depends on the contract shape of grounding, not on
  `keiko-local-knowledge` internals; the server binds plan to execution, preserving the existing
  grounding trust boundary (ADR-0022/0034/0036).
