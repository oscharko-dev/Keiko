...
| Wire-safe domain contracts, validators, branded ids, schema versions | `keiko-contracts`: `memory.ts`, `connected-context.ts`, `workflow-handoff.ts`, `evidence.ts`, `gateway.ts`                                                                         |
