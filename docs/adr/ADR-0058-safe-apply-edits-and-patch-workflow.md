...
A Playwright browser test in `tests/e2e/editor-agent-1394.spec.ts` verified undo/redo at the time of this decision. (Housekeeping 2026-08-10: the per-issue agent suites 1394/1395 were retired after months without a running lane and with assertion drift against the current surface. Their general editor and agent-changeset flows are exercised by the consolidated `editor-baseline-1377` regression set and the code-task authority journey. The 1395 denied-path governance/redaction proof is restored as a targeted pin in `tests/e2e/editor-agent-pins.spec.ts`, wired into the `e2e-extended` matrix, and extended with the M11 authority-missing fail-closed default. The browser undo/redo round-trip is still without an e2e pin: the product loses the undo history across the agent-review remount — tracked with full evidence and a ready test draft in issue #3070. Two contributing keiko-editor defects, the undo-clearing `setValue` value sync and the duplicate undo stop on identical host edits, are fixed and unit-pinned in `KeikoCodeEditor.test.tsx`.) The original assertion verified undo/redo
