...
A Playwright browser test in `tests/e2e/editor-agent-1394.spec.ts` verified undo/redo at the time of this decision. (Housekeeping 2026-08-10: the per-issue agent suites 1394/1395 were retired after months without a running lane and with assertion drift against the current surface. Their general editor and agent-changeset flows are exercised by the consolidated `editor-baseline-1377` regression set and the code-task authority journey. Both invariants are restored as targeted pins in `tests/e2e/editor-agent-pins.spec.ts`, wired into the `e2e-extended` matrix: the 1395 denied-path governance/redaction proof (extended with the M11 authority-missing fail-closed default) and the browser undo/redo round-trip across an agent-applied edit — proven in the mounted editor for the first time (the original suite could only assert 202/queued). The round-trip is backed by undo-preserving programmatic writes across keiko-editor, each unit-pinned: the controlled value sync, host-edit requests, and the retained-model registry's re-attach catch-up write (#3070) all go through the edit-operations API instead of the history-clearing `model.setValue`.) The original assertion verified undo/redo
