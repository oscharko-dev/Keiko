# ADR-0060: Governed Git Approval and Preview Surface
...
Issue #471 (ADR-0058) delivered the typed contract surface: action kinds, a risk taxonomy, the
lifecycle envelope, policy packs with a deterministic `evaluateGitPolicy`, and provider-neutral
interfaces. Issue #472 (ADR-0059) built the execution and preflight kernel that consumes those
...
- ADR-0058: Governed Git delivery contracts (the contract surface this projection consumes)
- ADR-0059: Governed Git mutation execution kernel (the preflight and lifecycle facts this surface projects)
