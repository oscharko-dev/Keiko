# ADR-0060: Realtime voice transport — re-opened loopback WebSocket control + browser WebRTC media
...
[ADR-0058](ADR-0058-voice-digital-twin-capability-architecture.md) (D3) and
[ADR-0059](ADR-0059-voice-control-media-capability-replay-protocol.md) (D3) established that "WebSocket
...
### D3 — Security posture for the re-opened upgrade (ADR-0058 D6)
...
### D4 — Existing strict controls: re-justified, not silently relaxed (ADR-0058 D6)
...
- **Supply chain.** No new runtime media package is added; the control plane reuses the existing `ws`
  dependency and the media plane uses browser-native WebRTC. The #496 supply-chain denylist test stays
  green.
...
- The re-opened upgrade is a deliberate, reviewed trust-boundary change confined to one loopback path
  ...
  This ADR is the security re-justification the gate (ADR-0058 D6) requires.
- ADR-0059 is promoted to **Accepted**: its protocol is now realized by transport. The contract baseline
...
- [ADR-0058](ADR-0058-voice-digital-twin-capability-architecture.md) (D3 transport, D6 security),
  [ADR-0059](ADR-0059-voice-control-media-capability-replay-protocol.md) (the protocol contract).
