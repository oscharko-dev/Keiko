# ADR-0062: Governed Local Git Flows and Commit-Intent Composition
...
- ADR-0059 (#472): mutation kernel — single execution authority (`runGitMutation`), preflight evaluators, narrow adapter port, no-shell spawn boundary.
...
- **Cons**: `git switch` is a mutation. Routing it through the read-only terminal allowlist would either require widening that allowlist (explicitly prohibited by ADR-0058 D7 and Force 6) or introduce an ungoverned execution path outside the kernel's authority (prohibited by ADR-0059 D1). Either path breaks a hard invariant of the governed delivery architecture.
...
- **Why rejected**: The preview route is the architectural device that keeps policy validation outside the kernel while delivering feedback before execution. It is the same pattern as `POST /api/git-delivery/action-sheet` (ADR-0060 D2): read-only, projection only, no side effects.

## Related

- ADR-0058: Governed Git delivery contracts (action kinds, risk taxonomy, lifecycle envelope — extended by one kind here)
- ADR-0059: Governed Git mutation execution kernel (`runGitMutation`, preflight dispatch, adapter port — extended by one kind here)
- ADR-0060: Governed Git approval and preview surface (action-sheet BFF pattern reused for preview route; `isGitDeliveryTrusted` gate reused)
- ADR-0061: Governed Git mutation evidence ledger (`recordGitDeliveryMutationEvidence` reused for all five new routes)
