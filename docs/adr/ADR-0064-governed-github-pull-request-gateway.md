# ADR-0064: Governed GitHub Pull Request Gateway and Metadata Orchestration
...
- ADR-0058 (#471): typed contract surface — action kinds, risk taxonomy, lifecycle envelope, policy evaluator.
- ADR-0059 (#472): mutation kernel — single local execution authority, preflight evaluators, narrow local adapter port.
- ADR-0060 (#473): approval and preview presentation layer — content-free action sheet, BFF projection route.
- ADR-0061 (#474): evidence ledger — bounded append-only record for every terminal outcome, audit-export route.
...
- `GitPullRequestAdapter` — the narrow provider port with two typed methods (`createPullRequest(req)` and `updatePullRequest(req)`). No generic `run(args)` escape hatch; the port accepts only the typed request shapes, mirroring the ADR-0059 and ADR-0063 narrow-port discipline.
...
Force-push for a PR update (e.g. force-updating the head branch after a rebase) is out of scope for this slice. The PR gateway has no force operand; any head-branch force push must use the ADR-0063 publish gateway with a future explicit policy path.
...
- ADR-0058: Governed Git delivery contracts (PR input shapes, execution error codes, recovery vocabulary reused unchanged; provider-neutral PR state interfaces reused)
- ADR-0059: Governed Git mutation execution kernel (preflight pr-create/pr-update → `preflightNoLocalPrecondition` unchanged; lifecycle result shape reused)
- ADR-0060: Governed Git approval and preview surface (read-only BFF preview pattern; `isGitDeliveryTrusted` gate reused; action-sheet recovery vocabulary reused)
- ADR-0061: Governed Git mutation evidence ledger (`recordGitDeliveryMutationEvidence` / `buildGitDeliveryEvidenceRecord` pr-create/pr-update projection reused)
- ADR-0062: Governed local Git flows (GovernedGitFlowCard extended with "PR" launch button in Publish section)
...
- ADR-0065: (next on this feat branch)
