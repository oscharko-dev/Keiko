# ADR-0065: Governed Merge Gateway, Protected-Branch Enforcement, and Guided Recovery
...
- ADR-0058 (#471): typed contract surface — action kinds, risk taxonomy, lifecycle envelope, policy evaluator. The `merge` action kind, `GitDeliveryMergeInputs`, `GitDeliveryMergeStrategyHint`, and the `GitDeliveryMergeBlockReason` taxonomy were defined here but never wired into an execution authority.
- ADR-0059 (#472): mutation kernel — single local execution authority, preflight evaluators (`merge` already maps to `preflightNoLocalPrecondition`), narrow local adapter port.
- ADR-0060 (#473): approval and preview presentation layer — content-free action sheet, BFF projection route.
- ADR-0061 (#474): evidence ledger — bounded append-only record for every terminal outcome, audit-export route. Its `buildGitDeliveryEvidenceRecord` is generic over the action kind (the `merge` envelope flows through unchanged).
- ADR-0062 (#475): first end-user-visible local flows — branch / staging / commit with interactive preview and commit-intent composition.
- ADR-0063 (#476): governed publish gateway — dedicated remote push authority with its own allowlist, rejection taxonomy, and policy pack.
- ADR-0064 (#477): governed pull request gateway — dedicated `gh api` PR-orchestration authority (create / update / draft-toggle), readiness model, rejection taxonomy. Explicitly deferred merge execution to this slice.
...
- A third `gh api`-shelling executor adds subprocess surface. `gh` must be installed and authenticated; absence is classified at execution time (no pre-flight `gh` probe), consistent with ADR-0063/0064.
...
- **Cons**: The PR allowlist explicitly excludes merge/delete (ADR-0064 D1); the PR port has create/update methods only; the merge failure taxonomy (405/409/422-required-checks) is distinct. Adding merge would erode the PR gateway's no-merge invariant and widen its allowlist.
...
- **Why rejected**: Force 4 / ADR-0064 Alternative 2. The `gh api` subprocess preserves token isolation at the process boundary.

## Related

- ADR-0058: Governed Git delivery contracts (`merge` action kind, `GitDeliveryMergeInputs`, `GitDeliveryMergeStrategyHint`, `GitDeliveryMergeBlockReason`, provider-state interfaces reused unchanged)
- ADR-0059: Governed Git mutation execution kernel (`merge` → `preflightNoLocalPrecondition` unchanged; lifecycle result shape reused)
- ADR-0060: Governed Git approval and preview surface (read-only BFF preview pattern; `isGitDeliveryTrusted` gate reused)
- ADR-0061: Governed Git mutation evidence ledger (`buildGitDeliveryEvidenceRecord` records the merge envelope unchanged)
- ADR-0063: Governed remote publish gateway (parallel gateway pattern mirrored; publish gateway unchanged)
- ADR-0064: Governed pull request gateway (parallel gateway pattern mirrored; PR gateway unchanged; merge was deferred to this slice)
