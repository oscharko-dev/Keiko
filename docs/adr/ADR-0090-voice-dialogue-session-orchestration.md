# ADR-0090: Voice dialogue session orchestration — deterministic turn controller, STT+TTS fallback, barge-in, and master cleanup
...
- **Issue #494/#495 (ADR-0058 D4)** shipped `useDictation` — the controlled STT capture state machine
  ...
  user-capture path.
- **Issue #1558 (ADR-0089)** shipped `useAssistantSpeech` — the audio playback engine that synthesizes
...
- [ADR-0089](ADR-0089-voice-assistant-speech-synthesis.md): the `useAssistantSpeech` playback engine and
