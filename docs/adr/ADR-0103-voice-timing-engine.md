# ADR-0103: Voice timing engine — local monotonic timebase, bounded buffers, backpressure, and catch-up

> Renumbered from ADR-0061 on 2026-07-04 to resolve the 0058-0069 editor/voice numbering collision (Epic #491 voice series moved to 0100-0111).
...
[ADR-0102](ADR-0102-realtime-voice-transport.md) realizes the **host-side** transport — the
...
transport. It is the symmetric, client-side mirror of the server reconciliation that ADR-0102 already
ships.
...
(ADR-0102) and the two planes are deliberately symmetric, not shared. `keiko-ui` is excluded from the
...
`VOICE_TIMEBASE_REPLAY_CAPACITY = 200` (matching the server's `MAX_REPLAY_EVENTS`, ADR-0102). Only
...
is the client mirror of the server's bounded replay (ADR-0102 D3). Catch-up replays *control state*,
...
The envelope's per-direction `seq` is monotonic (ADR-0101). The engine tracks the highest applied
...
cannot silently drift (privacy-contract §2 / §3, ADR-0100 D6).
...
- **Why rejected**: the contract is frozen pure data + validators (ADR-0101); the engine is a stateful
...
- **Cons**: raw audio is media-plane, never persisted or replayed (epic invariant, ADR-0101 D5),
...
- [ADR-0100](ADR-0100-voice-digital-twin-capability-architecture.md) (capability gating, D6 security),
  [ADR-0101](ADR-0101-voice-control-media-capability-replay-protocol.md) (protocol, replay / redaction
  classes, gating table), [ADR-0102](ADR-0102-realtime-voice-transport.md) (server reconciliation,
