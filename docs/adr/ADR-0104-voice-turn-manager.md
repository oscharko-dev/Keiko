# ADR-0104: Voice Turn Manager — floor control, barge-in, and end-of-turn semantics

> Renumbered from ADR-0062 on 2026-07-04 to resolve the 0058-0069 editor/voice numbering collision (Epic #491 voice series moved to 0100-0111).
...
[ADR-0100](ADR-0100-voice-digital-twin-capability-architecture.md) makes voice optional and
capability-gated; [ADR-0101](ADR-0101-voice-control-media-capability-replay-protocol.md) defines the
wire protocol including `control.interrupt`, `playback.state`, `media.track.state`,
`transcript.committed`, and `transcript.discarded`; [ADR-0102](ADR-0102-realtime-voice-transport.md)
realizes the server-side WebSocket control plane and the bounded replay buffer; and
[ADR-0103](ADR-0103-voice-timing-engine.md) ships the client-side timing engine
...
- The [#498 timing engine pattern](ADR-0103-voice-timing-engine.md): factory + injectable clock +
...
`createBrowserVoiceClock` from `voice-timebase.ts` via relative path — the convention ADR-0103
...
### D2 — Factory `createVoiceTurnManager({ profile, clock?, observer? })` mirrors ADR-0103 exactly (AC1 / AC6)
...
truth; the manager maintains no parallel gating table (ADR-0101 is the authority). Admission keys on the
...
(architecture.md §6, ADR-0100 D1), which the turn manager has no path to reach or bypass.
...
state / effect / observer traces — exactly the pattern ADR-0103 established. Named fixture scenarios
...
- Like ADR-0103, this module deliberately mirrors the shape of the server's floor/idempotency logic
  without sharing code; the contract is the coupling point.

## Alternatives Considered

### Alternative 1: Embed floor control inside the timing engine (ADR-0103)
...
- **Why rejected**: separation of concerns. The timing engine can be unit-tested without floor
  ...
  ADR-0103 D10 explicitly defers "semantic rendering wiring" to later issues, making the boundary
  clear.
...
- **Cons**: impossible to unit-test without a React test environment; cannot be shared with a future
  non-React consumer; the "synchronous reducer, no hidden async queues" mandate (ADR-0103 D3) is
...
- **Cons**: `VOICE_PROFILE_ALLOWED_MESSAGE_KINDS` in `keiko-contracts` is the authoritative table
  (ADR-0101); duplicating it introduces drift risk. ADR-0103 D8 explicitly rejects this for the
...
- **Why rejected**: single source of truth. Gating is delegated to `voiceMessageAllowedForProfile`
  from the contract, exactly as ADR-0103 does.

## Related

- [ADR-0100](ADR-0100-voice-digital-twin-capability-architecture.md): capability gating; D1 (no new
  authority); D6 (security review contract)
- [ADR-0101](ADR-0101-voice-control-media-capability-replay-protocol.md): the protocol kinds and
  `VOICE_PROFILE_ALLOWED_MESSAGE_KINDS` gating table this ADR delegates to
- [ADR-0102](ADR-0102-realtime-voice-transport.md): server-side control plane, bounded replay buffer,
  seq/idempotency
- [ADR-0103](ADR-0103-voice-timing-engine.md): the timing engine this module sits above; the
