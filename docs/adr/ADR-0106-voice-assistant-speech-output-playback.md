# ADR-0106: Voice assistant speech-output playback — optional capability-gated playback lifecycle, controller, and interruption boundary

> Renumbered from ADR-0064 on 2026-07-04 to resolve the 0058-0069 editor/voice numbering collision (Epic #491 voice series moved to 0100-0111).
...
`playback.state` and `control.interrupt`; [ADR-0102](ADR-0102-realtime-voice-transport.md) realizes the
server-side control plane; [ADR-0103](ADR-0103-voice-timing-engine.md) and
[ADR-0104](ADR-0104-voice-turn-manager.md) ship the client timing engine and floor-control turn manager;
[ADR-0105](ADR-0105-voice-transcript-segment-semantics.md) ships the transcript segment lifecycle.
...
(`preparing`), provider failure (`failed`), and a natural end (`complete`) — exactly as ADR-0104 and
ADR-0105 map a richer semantic set onto the same coarse wire catalog.
...
impact. This mirrors the ADR-0105 contract/reducer split.
...
truth — rather than re-encoding the profile table. By the ADR-0101 table, `none` and `speech-to-text`
...
`voicePlaybackEffectToTurnSignal` maps these onto the ADR-0104 turn-manager signal union, and
...
### Alternative 2: Embed playback state in the turn manager (ADR-0104)
...
- [ADR-0100](ADR-0100-voice-digital-twin-capability-architecture.md),
  [ADR-0101](ADR-0101-voice-control-media-capability-replay-protocol.md),
  [ADR-0102](ADR-0102-realtime-voice-transport.md), [ADR-0103](ADR-0103-voice-timing-engine.md),
  [ADR-0104](ADR-0104-voice-turn-manager.md), [ADR-0105](ADR-0105-voice-transcript-segment-semantics.md).
