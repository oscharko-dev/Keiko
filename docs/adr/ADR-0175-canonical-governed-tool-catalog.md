...
Its `owners`, `axes`, `interfaces`, `statuses`, `phases`, `bounds` and `consumers` sections are checked
...
`InvocationReceipt` carries `settlementId`, `budgetDisposition` (`committed`, `released` or `not-reserved`),
...
| Completed key replay | Return existing immutable settlement/receipt; no second event, effect or charge |
...
catalog/profile/projection/handler identities. A feature-branch SHA, issue status or cherry-pick
assumption cannot replace that checkpoint. Durable H1 provenance survives removal of the temporary
pending-H1 migration entry and never enters a semantic projection digest.
...
ownership and deferred scopes. The machine table contains reproducible file/token probes, complete
producer/consumer interface mappings and negative consistency tests. #3406 introduces the actual
finite shrink-only migration guard; #3415 expands it and removes all parallel definitions and the
