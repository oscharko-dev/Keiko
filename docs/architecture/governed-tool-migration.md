...
The final #3415 migration state accepts only the bound `ToolPort`/`GatewayRequest` arm.
Name-only input, both arms together, implicit latest resolution, downgrade and cross-profile
replay remain invalid. The earlier seven-day legacy bridge described the transition phase; it
is not a capability to reintroduce after every consumer has migrated. The source-derived active
migration register is empty, and negatives prevent restoration of the retired arm. The immutable
43-row architecture census remains available for ownership history.

The temporary, **non-authorizing** `pendingH1` entry introduced by #3406 carried the #3386 owner,
canonical search identity, #3411 prerequisite, #3414 removal owner and initially empty landing
fields. It never granted an allowlist or dispatch exception. After PR #3394 landed and the durable
record below was available, #3414/#3415 removed `pendingH1` from the generated
[tool-catalog-migration.v1.json](tool-catalog-migration.v1.json), as its finite migration contract
requires. `GOVERNED_TOOL_CONTRACT_PINS.h1Provenance` is not a surviving migration row: it retains
only the dev merge/digest pins and dependency-closure seeds needed for permanent independent
revalidation. Runtime authority and projection code do not read it.
...
inside `checkToolCatalogMigrationCloseout`. The historical checkpoint record at
...
references pin the SHA-256 of retained body-free receipts. Each receipt must accept the same source
head and owned-source digest.
...
The source-set entry points are `h1Provenance.ownedImplementation` in the existing canonical pins.
...
The separately retained post-merge validator, `checkH1HandoffEvidence`, requires the stable
`landedDevCommit` and `landedTreeDigest` pins together. It checks the actual
...
Repository unit tests use hermetic Git histories; they do not fetch from the network. No pre-merge
checkpoint claims that final required checks or a GitHub merge have already happened. The integrator records those
facts only after the native merge and subsequent dev verification. Provenance does not enter
runtime authority decisions or semantic projection digests.
