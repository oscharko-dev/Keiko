...
| Workspace search, scan, raw reader and Editor selector    | workspace-search, workspace-scan, workspace-raw-read, editor-raw-lane  | Retain owner; #3386 adds H1 only after ADR-0165 gate merge; #3408 resolves private Editor search routes              |
...
| #3406    | Merged #3411 contract                                                     | contracts + pure catalog package/compiler/manifest; initial finite registry guard and package DAG/exports                                                      |
| #3412    | Merged #3411 phase vocabulary                                             | Existing operation scanner, generated operation artifact/provenance and phase fixtures; no runtime emitter/analyzer edits                                      |
| #3386 H1 | Merged #3411 raw-coordinate gate; reserved `keiko.repo.search@1` identity | Existing workspace implementation and server typed handler/readiness/containment/cancellation/tests; no aliases, schema projections, prompt, IPC or allowlists |
...
| #3414    | #3409 bound arm + #3413 binder + merged H1                                | OpenCode source/launch/profile/protocol/IPC/prompt/config and semantic digest; durable H1 provenance                                                           |
...
`keiko.repo.search@1`, #3411 actual merged-dev identity and removal issue #3414. `landedDevCommit`
and `landedTreeDigest` start empty. No model-visible file, alias, schema, effect, dispatch table or
consumer is allowed by this entry. #3414 verifies GitHub's actual H1 mergeCommit/tree and required
source-head checks, writes durable H1 provenance, then removes the temporary entry. Durable
provenance survives, is independently revalidated by #3415, and is not a projection-digest input.
...
Required checks must pass on the exact PR head; #3411 stays open until verified merge. Its merge
identity/tree (not its branch head) is the handoff to H1 and the mandatory catalog children.
