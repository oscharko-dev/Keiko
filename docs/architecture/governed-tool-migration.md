...
| Native provider projection and Realtime compatibility     | gateway-openai, realtime-compatibility                                 | #3409 derives supported projections; #3415 removes the unused provider-native Realtime tool request types while retaining the media/transcription negotiation owner                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
Known gap on this head: the old-arm ACCEPT path described above is not yet implemented anywhere in
the tree — `packages/keiko-tool-catalog/src/invocation.ts`'s normalizer unconditionally rejects
`kind: "legacy-name"` input (see `retiredBridgeMigrations`'s own probe for that exact literal,
`scripts/check-tool-catalog-conformance.mjs`), and `packages/keiko-contracts/src/governed-tool-bridge.ts`'s
`GatewayToolCatalogAdvertisement` type accepts no advertisement `kind` but `"bound"`. This is
safer than the gap it creates (fail-closed, never a forged legacy call), but it means #3415 has no
legacy arm left to delete when it closes out — the finite migration inventory already reads zero
without one ever having existed. Implementing the accept path requires widening
`governed-tool-bridge.ts`'s type (and its own pin in `governed-tool-bridge.test.ts`), which is
outside `invocation.ts`'s and `toolCatalogBridge.ts`'s own ownership boundary; tracked as open
#3409 AC7 scope, not silently dropped.

The pending-H1 entry introduced by #3406 is separate and **non-authorizing**. It is the `pendingH1`
object in [tool-catalog-migration.v1.json](tool-catalog-migration.v1.json) (generated by
`toolCatalogMigrationBytes` in `scripts/check-tool-catalog-conformance.mjs` from
`GOVERNED_TOOL_CONTRACT_PINS.pendingH1` in `scripts/lib/governed-tool-contract-pins.mjs` — never
hand-edited) and carries exactly six fields: `owner` (#3386), `canonicalTool`
...
#3415's independent revalidation is `checkH1HandoffEvidence`
(`scripts/check-tool-catalog-conformance.mjs`, folded into `checkToolCatalogMigrationCloseout`,
so it runs on every `--closeout` invocation). While `pendingH1.landedDevCommit`/`landedTreeDigest`
stay honestly null (no dev-landing has happened on this head), it is a no-op: `[]`, never a
fabricated pass. The moment either field is populated it independently rechecks, never trusting a
caller-declared value alone: both fields must be set together and shape-valid (a 40-hex commit SHA,
a 64-hex tree digest); a durable `H1Provenance` record must exist at
`docs/architecture/h1-provenance.v1.json` (schema `H1Provenance` in
`scripts/lib/governed-tool-contract-shape.mjs`) and pass full shape validation; its `treeDigest`
and `currentHead` must agree with `pendingH1`'s own fields; `landedDevCommit` must be a real,
