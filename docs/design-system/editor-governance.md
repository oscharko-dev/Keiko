...
| Agent-in-editor visual primitives (banner / agent ranges / ghost text / proposal markers)                      | `editor-agent.html`                                                                                             | `--ed-agent-*` (accent-only, applied as editor decorations)                                                                                                                                                                          | Default proposal, Active/focused range, Loading (streaming), Syncing (multi-file review marker), Conflict (edit vs dirty buffer marker), Error (governed/blocked), Empty (no proposals)             | Browser/Playwright of token-backed banner / ranges / ghost text / proposal-review markers in Dark/Light/HC, asserting brand-green-only and attributable ranges. Live hunk accept/reject, inline chat, provenance hover, and authority-bearing apply flows require separate #1390/#1405 evidence before they can be claimed. | visual/token fidelity owned by [#1299](https://github.com/oscharko-dev/Keiko/issues/1299); agent authority/audit remains deferred to [#1390](https://github.com/oscharko-dev/Keiko/issues/1390) / [#1405](https://github.com/oscharko-dev/Keiko/issues/1405) |
...
- Epic [#1390](https://github.com/oscharko-dev/Keiko/issues/1390) (Agent-Native Editor API and Workspace Governance)
  ...
  #1390's future audit / evidence contracts, but do not claim those authority-bearing behaviours are already shipped.
- Issue [#1405](https://github.com/oscharko-dev/Keiko/issues/1405) owns live human-in-the-loop AI controls such as
  permission requests, sensitive-action confirmation, and regenerate wiring. #1299 may document the visual primitives
  only; live behaviour requires #1405 evidence.

Stop condition: #1299 must **not** promise agent behaviour that #1390 has not implemented. Documentation must not
promise components or states the implementation does not support; the agent-in-editor row above is a visual/token
contract with explicit deferrals for authority-bearing behaviour, not a forward commitment.
