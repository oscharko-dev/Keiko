...
const MIME = { ".html":"text/html",".js":"text/javascript",".css":"text/css",".json":"application/json",".svg":"image/svg+xml",".png":"image/png",".ico":"image/x-icon",".webmanifest":"application/manifest+json",".woff2":"font/woff2",".woff":"font/woff",".txt":"text/plain",".map":"application/json" };
const server = createServer(async (req, res) => {
  try {
    ...
    catch { try { buf = await readFile(fp + ".html"); fp += ".html"; } catch { res.writeHead(404); return res.end("nf"); } }
...
const MODES = [
  ...
  { id: "dark-hc",        theme: "dark",  hc: "more", media: { forcedColors: undefined, contrast: "more" } },
...
for (const vp of VIEWPORTS) {
  for (const mode of MODES) {
    ...
    for (const route of ROUTES) {
      ...
      await page.addInitScript((t) => { try { localStorage.setItem("keiko.theme", t); } catch {} }, mode.theme);
      ...
      manifest.push({ file: name, route: route.path, viewport: vp.id, mode: mode.id, ...info, pageErrors: errs.slice(0, 3) });
      shotCount++;
...
writeFileSync(join(OUT_DIR, "manifest.json"), JSON.stringify({ issue: 1295, shotCount, manifest }, null, 2));
