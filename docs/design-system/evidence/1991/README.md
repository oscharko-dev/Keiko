...
> **Historical / not reproducible by a lane.** The governed-assist authority @smoke spec
> `tests/e2e/coding-workbench-1991.spec.ts` was retained but its evidence-writing body was
> removed in earlier housekeeping, so `KEIKO_WRITE_TRACKED_EVIDENCE=1 npm run
> test:e2e:coding-workbench-1991` no longer regenerates the artifacts below. The tracked captures
...
> future shape audit. Live coverage for the workbench-authority surface: the extended e2e matrix
> suite `tests/e2e/coding-workbench-2253.spec.ts` and the governed-assist server-cap @smoke check
> retained in this same spec file, which continues to gate that the workbench honors the
> server-reported authority ceiling rather than a fixture projection.

Historical (non-regenerating) invocation, kept for shape context only:

```bash
npm run test:e2e:coding-workbench-1991   # @smoke checks only; regenerates no evidence in this directory
```
