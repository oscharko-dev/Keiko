...
- `/launch` and `/local-knowledge` re-export `KeikoDesktop`; the six `/memoriaviva/*` route stubs were
  deleted in #2894 (KEIKO-0129) — bookmarked deep links fall through the server's `serveStatic`
  index.html fallback onto the desktop shell instead of the previous JS-only redirect. The live
  memoriaviva surface remains the in-desktop `memoria` window (owned by #1295).
  `/memoriaviva/detail/MemoryDetailClient.tsx` remains on disk as a still-unrouted orphan.
...
| `/memoriaviva` (+ five sub-paths) | Deleted in #2894 (KEIKO-0129); falls through to `serveStatic` index.html on the desktop shell | (no route files; `app/memoriaviva/components/*` still owns the in-desktop `memoria` window)                                                 | #2894 (fix) / #1295 (live) |
...
| Coding Workbench (`coding`)                                                                                                                                 | `widgets/coding-workbench/{CodingWorkbenchWindow,CodingWorkbenchSections,CodingWorkbenchModelCards}.tsx`, `lib/{useCodingWorkbenchRuntime,coding-workbench-live-state,coding-workbench-runtime-api,coding-workbench-event-retention}.ts`, `windows/WindowsRegistry.ts` | loading / empty / unavailable / error / idle / running / approval-required / recovery-required / failed / completed / governed-assist / supervised-coding / autonomous-delivery / density full·820·768·560·390·320 / dark·light·high-contrast·forced-colors·reduced-motion | #1982 / #2257                     |
...
| --- | -------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D1  | `/memoriaviva/*` route stubs + orphaned `MemoryDetailClient.tsx`                             | Route stubs deleted in #2894 (KEIKO-0129); `serveStatic` falls through to the desktop shell on bookmarked deep links. `MemoryDetailClient.tsx` remains a stand-alone orphan tracked separately. The live surface is still the in-desktop `memoria` window (owned by #1295).                                                                                                                                                                                                                                                                 |
