...
[packages/keiko-ui/src/app/globals.css](../../packages/keiko-ui/src/app/globals.css) (22,658 lines).
...
a code comment at `globals.css:22231` ("C415 — `.ws-fab` keeps a hardcoded 50%-black drop shadow in light theme")
...
| --- | ---------------------------------------------------------------- | -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| A1  | Select/tooltip popover `.ksel-menu`                              | `globals.css:685-696`                  | shadow now `rgb(var(--shadow-ink-rgb) / 0.85)` (mode-aware ink) — resolved. Residual `color-mix(--popover-surface 98%, black)` bg tint (line 691) is a tracked approved deviation                                         | resolved required (residual bg tint = approved deviation) | theme=light; open any KeikoSelect dropdown (model picker, source-kind selectors)                                                                 |
| A2  | Header tool CTA `.hd-tool-cta`                                   | `globals.css:1842-1860`                | shadow now `rgb(var(--shadow-ink-rgb) / 0.75)` (mode-aware) — resolved. Retains the white inset bevel `oklch(1 0 0/0.04)` (approved deviation, invisible-but-inert in light)                                              | resolved required                                         | theme=light; header tool CTA in top bar                                                                                                          |
| A3  | Workspace zoom control `.ws-zoom`                                | `globals.css:2236-2252`                | shadow now `rgb(var(--shadow-ink-rgb) / 0.9)` (mode-aware) — resolved. Retains white inset bevel (approved deviation)                                                                                                     | resolved required                                         | theme=light; workspace zoom % control                                                                                                            |
| A4  | Composer model menu `.cmp-model-menu`                            | `globals.css:3459-3463`                | shadow now `rgb(var(--shadow-ink-rgb) / 0.8)` (mode-aware) — resolved                                                                                                                                                     | resolved required                                         | theme=light; open model selector in chat composer                                                                                                |
| A5  | QI source-kind tooltip `.qi-source-kind-option[data-tip]::after` | `globals.css:17381-17405`              | shadow now `rgb(var(--shadow-ink-rgb) / 0.85)` (mode-aware) — resolved. Residual `color-mix(--popover-surface 96%, black)` bg tint (line 17393) is a tracked approved deviation                                           | resolved required (residual bg tint = approved deviation) | theme=light; hover a QI source-kind option with a tooltip                                                                                        |
| A6  | Chat brand logo `.chat-msg-brand img`                            | `globals.css:9578-9600`                | `drop-shadow(… rgb(var(--shadow-ink-rgb) / 0.72))` (static + keyframes) now mode-aware — resolved                                                                                                                         | resolved required                                         | theme=light; open a chat thread; brand avatar (animated when `data-pulsing=true`)                                                                |
| A7  | Workspace FAB base `.ws-fab`                                     | `globals.css:7411-7427`                | base shadow now `rgb(var(--shadow-ink-rgb) / 0.55)` (mode-aware) — resolved. The C415 `[data-theme="light"] .ws-fab` override (line 22233) is retained as an intentional per-mode density correction (approved deviation) | resolved required (light override = approved deviation)   | theme=light; inspect both `.ws-fab` rules in devtools                                                                                            |
| A8  | Generation-pref slider thumb `.gpref-slider` thumb               | `globals.css:6258-6269`                | `box-shadow … rgba(0,0,0,0.35)` on `::-webkit/::-moz` thumbs; mild over-darkening, low impact                                                                                                                             | approved deviation                                        | theme=light; open generation preferences slider                                                                                                  |
| A9  | Window frame inset highlight `.win` frame                        | `globals.css:1583-1600`                | inset `oklch(1 0 0/0.025)` top-bevel assumes dark surface; invisible (dead style) in light                                                                                                                                | approved deviation                                        | theme=light; inspect a workspace window frame top edge                                                                                           |
| A10 | Workflow log wells + Figma image preview                         | `globals.css` (`color-mix` well group) | `color-mix()` over `--bg`/`--card` (88-92%) darkened with black is theme-blind → flat light-gray well in light                                                                                                            | approved deviation                                        | theme=light; open a workflow run log well / Figma image window                                                                                   |
...
| FAB / primary shadow          | `.ws-fab` (1381 and 22233), `.dlg-primary` (1382), `.cmp-send[data-on]` (1383)                                                                                                                                                                                                                                                                               | C415 shadow/fill patches that paper over hardcoded dark shadows                              | **required** (route through `--shadow-*`)                                                                                    |
...
Every known Light Mode mismatch is classified: Table A (12 visible defects: 8 required at audit time — the
forced-colors gap (A12) plus the seven hardcoded-dark-shadow rows A1–A7 are now **resolved**, since #1295 routed
every one of those shadows through the mode-aware `rgb(var(--shadow-ink-rgb) / α)` primitive so Light adapts
automatically; the only residuals are the two approved `color-mix(…, black)` popover-surface tints on `.ksel-menu`
(A1) and the QI tooltip (A5), tracked as approved deviations — and 4 approved deviations) and Table B (all 49
component-scoped `[data-theme="light"]` overrides: 10 required, 39 approved deviations), plus the related blocking
`z-index` governance item. No `[data-theme="light"]` rule and no hardcoded-dark-shadow surface is left
unclassified. Reproduction steps are provided for every row in Table A; Table B rows reproduce via the same
theme-toggle harness and the cited selector.
