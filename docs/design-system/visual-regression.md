...
| High-traffic surfaces | chat / QI / grounded / Local Knowledge / memoria / relationships / files / editor / Git | semantic tier               | seeded app workspaces | #1294/#1295 | 6     |
...
| Seeded high-traffic workspaces                         | running app           | Deterministic `keiko.workspace.v4` captures cover chat / Quality Intelligence / Local Knowledge, MemoriaViva / Relationships, Files / Editor, and both Git-client sizes across all viewports and modes. | none                                   | ✅ accept                     |
...
| V2  | 0.4.0 editor tokens `--ed-inlay-fg`, `--ed-blame-fg`, `--ed-fold-placeholder` declared in reference; inlay foreground/background and debug-value variants are now adopted on the editor runtime component scope, while blame/fold remain reference-only | inlay hints, inline debug values, git blame, fold placeholder                                                                           | component-scoped adoption for inlays; remaining variants reference-only | **Partially adopted** — Foundation-wave integration supplies inlay tokens without modifying SHA-locked global CSS; blame/fold remain owned by their editor surfaces | Global proof remains `referenceOnly`; `EditorRuntimeWidget.theme-tokens.test.ts` pins the component-scoped inlay adoption. |
