...
- the exact canonical sanitized report a user approved and its paired local disposition state;
...
| Local content -> previewed report      | Potentially secret/customer-bearing text                                             | Strict accepted-text gates; structural detection; exact-span redaction or smallest-safe-unit quarantine; optional omission; source bytes/metadata stay local. |
| Loopback BFF -> hosted anonymous route | Exact approved canonical sanitized bytes                                             | One configured HTTPS origin, fixed `POST /v1/feedback/reports`, `gatewayFetch`, no credentials, redirect denial, timeout/response caps.                       |
...
| Anonymous handler -> queue             | Untrusted claimed schema/digest/provenance                                           | Independent parse, canonical-byte/digest/version/limit validation and residual scan; reject drift rather than redispose or mutate.                            |
...
| Tampering              | A client lies about provenance or sends content that still requires redaction/quarantine.             | Provenance is informational and untrusted. Hosted validation supports only known versions and reruns residual rules; any remaining detection is drift and is rejected before persistence rather than silently redisposed.                                                                                                                 |
| Tampering              | Separately safe title and description values create a credential or raw-log signature when joined.    | Keep `summary.title` and `summary.description` distinct in the V1 canonical report; scan and hosted-verify each independently. Enforce one deterministic combined byte cap including the reserved display separator, and render title before description downstream without rescanning a concatenation.                                   |
| Tampering              | Stale recovery metadata focuses the wrong draft unit or an optional omission restores source content. | Pair the immutable local sidecar with the canonical-body digest. Regenerate both after every edit. Public/submit mappers read only canonical safe fields; they cannot read the draft, sidecar content, or omitted units.                                                                                                                  |
...
| Information disclosure | Secrets, customer data, paths, private logs, or attachment metadata leave the machine.                | Enforce text gates, redact proven complete spans, and quarantine an ambiguous quoted/structured value or line remainder without preserving fragments. Omit exhausted optional units; require a rewrite only when required safe content is empty. No source bytes/metadata cross; hosted intake validates again.                           |
| Information disclosure | A disposition notice leaks quarantined text, offsets, a filename, or a caller label.                  | Keep the sidecar local-memory-only and content-free: closed action/reason/unit-kind plus closed draft-field id or snapshotted attachment ordinal. Never serialize it into the wire body, public prefill, evidence, logs, diagnostics, or durable UI state.                                                                                |
...
| Denial of service      | Large bodies, separator storms, malformed nesting/quotes, or expensive Unicode exhaust resources.     | Enforce byte/depth/count caps, deadlines, bounded concurrency, fixed-pass linear scanning, bounded delimiter state, and no input-dependent backtracking. Reject binary/multipart/decompression and structurally unprocessable input early; performance-test maximum admitted sizes.                                                       |
| Denial of service      | Safe password-policy/reset prose is repeatedly quarantined, making reporting unusable.                | Credential ambiguity requires structural assignment/auth/userinfo evidence. Keep a safe natural-language regression corpus. Preserve safe remainder, localize omission, and offer edit/rescan without a `send anyway` bypass.                                                                                                             |
...
- **Dedupe identity:** service HMAC under the separate `keiko-feedback-dedupe-v1` domain over a
  version-tagged canonical projection of already-sanitized semantic report fields. It excludes abuse
...
private advisory route before ordinary report entry. Supported high-confidence vulnerability
indicators block submit/public-link actions and show the same route. Credential indicators instead
follow the report's exact-span-redaction or smallest-safe-unit disposition contract; a credential
word alone does not classify a report as a vulnerability. The hosted service and maintainer queue
repeat the private-routing control. A maintainer who encounters a suspected vulnerability
...
- A structurally ambiguous match can remove benign adjacent text. Deterministic smallest-safe-unit
  removal, natural-language negative fixtures, and field-level edit/rescan recovery reduce but do
  not eliminate this false-positive risk.
- A compromised local machine can observe the report before redaction. Local OS compromise is
  outside this feature boundary.
- The trusted TLS terminator and hosted operator can access the already-sanitized payload. Deployments
...
automatic public issue creation, a `send anyway` bypass, durable raw/quarantined content, or a
semantic safety claim the deterministic controls cannot prove.

## Required verification evidence

Issues #2072-#2077 must provide tests for exact preview/body/public-projection identity; canonical/
digest and server-rescan rejection; complete-span redaction; quoted/structured/line-remainder
quarantine; optional omission and required-empty rewrite; content-free draft-field/attachment targets;
idempotence; natural-language false positives; malformed-structure and maximum-size linear time;
strict UTF-8 scalar/control, binary magic, raw-log, multipart, and pre/post-size rejection; fixed
origin/path and redirect denial; Host/Origin/CSRF local guards; trusted/untrusted proxy chains, UTC
bucket start/close, exact start+48-hour expiry, current/previous abuse-key lookup/destruction, and
zero raw-IP logs; separate 90-day dedupe-key-ring/180-day entry expiry;
