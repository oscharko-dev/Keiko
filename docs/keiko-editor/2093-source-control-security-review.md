...
| Fixed local Git process profile          | `defaultGitProcessRunner` injects `-c core.fsmonitor=false -c core.editor=true`; `runner.test.ts` proves a repository-local executable fsmonitor is not invoked and the config overrides survive repository-local reconfiguration.                                                                                                                                                              |
| External diff/text conversion            | The single spawn boundary injects `--no-ext-diff --no-textconv` right after any diff-family subcommand (`diff`, `diff-files`, `diff-index`, `diff-tree`), so no future call site can omit them; blame uses `--no-textconv` at the call site; route tests and `runner.test.ts` assert a repository-local `diff.external` helper is never invoked while the internal-differ patch still succeeds. |
...
| Repository-configured external diff or textconv                                                                 | Runner injects `--no-ext-diff`/`--no-textconv` after every diff-family subcommand at the single spawn boundary, so no call site can omit them; helper never executes and the internal-differ patch still succeeds. | `gitRoutes.test.ts`, `runner.test.ts`                                   | Covered     |
