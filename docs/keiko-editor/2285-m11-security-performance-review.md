...
| `HISTORY-PLAINTEXT-LEAK`        | Search private vault/index/browser state for checkpoint bodies.                  | Vault bytes are ciphertext and browser storage contains no checkpoint content.             | `localHistoryStore.test.ts`, Playwright |
