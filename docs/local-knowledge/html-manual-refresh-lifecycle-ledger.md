...
| Changed page re-embed failure keeps previous vectors                                                    | `manual-pod-refresh.test.ts`: "keeps a changed page's previous vectors when its own re-embed fails"; `indexing/orchestrator.ts` per-document restore snapshots. |
...
### Resolved post-audit: indexing-phase failure does not erase a changed page's previous vectors
...
the indexing outcome is known. Two consequences of that ordering were audited post-merge and are
now handled explicitly:

- **No cross-failure pruning:** pages no longer in the new crawl's page set were previously pruned even when another
...
- **Per-document restore:** a changed page's previous chunks/vectors used to be deleted before its
  new content was successfully embedded. `indexing/orchestrator.ts` now captures a per-document
  restore snapshot before extraction/re-chunking and restores that snapshot if the same document
  fails during the run, so the page remains retrievable on its last known-good vectors. Regression:
  `manual-pod-refresh.test.ts` — "keeps a changed page's previous vectors when its own re-embed
  fails".

`source_scope_version` is also no longer inert bookkeeping: refresh accepts legacy rows with a
missing version, accepts the current version, and fails closed on any future unsupported value
before reconstructing a crawl scope.
...
| `npm run check:editor-release-evidence`                | Linux-authoritative. Post-audit verification in a temporary Linux `node:22-bookworm` copy passed with the committed fingerprint `8e39300b5a056f7eb72665e053d5785840475164826f8fa6c5551ae7eac531d6`; the macOS-local fingerprint is not authoritative and was not committed. |
...
| `npm run check:editor-release-evidence`                | Linux-authoritative. macOS-local measurement reports the known platform fingerprint difference after UI changes, but the same worktree copied to a temporary Linux `node:22-bookworm` container passed `npm ci`, `npm run build:packages`, `npm run build:ui`, and `npm run check:editor-release-evidence` with the committed fingerprint `8e39300b5a056f7eb72665e053d5785840475164826f8fa6c5551ae7eac531d6`. No macOS evidence JSON was committed. |
