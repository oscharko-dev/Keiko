...
| bounded scope preview                 | explicit limits, `followRedirects:false`, `estimatedPageCount:null`, denied reason-code labels | `documentation-manual-proposal.test.ts`, `docs-browser-proposal.test.ts`                                        |
...
| Editor release evidence    | historical (2026-07) | Post-audit verification in a temporary Linux `node:22-bookworm` copy — the then-governed runtime, superseded by the Node 24 line in #2294 — ran `npm ci`, `npm run build:packages`, `npm run build:ui`, and `npm run check:editor-release-evidence`; the committed Linux fingerprint `8e39300b5a056f7eb72665e053d5785840475164826f8fa6c5551ae7eac531d6` was current at the time of writing and is not the current fingerprint; `docs/release/1209-bundle-evidence.json` owns that value. The macOS-local fingerprint is not authoritative and was not committed. |
...
- **Denied link classes** are reason-code labels only before consent. Keiko has not crawled the
  target at this point, so it cannot truthfully report class counts without introducing pre-consent
  egress.
