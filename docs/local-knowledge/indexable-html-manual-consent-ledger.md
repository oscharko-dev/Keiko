...
| Editor release evidence    | Linux-authoritative | Post-audit verification in a temporary Linux `node:22-bookworm` copy ran `npm ci`, `npm run build:packages`, `npm run build:ui`, and `npm run check:editor-release-evidence`; the committed Linux fingerprint `8e39300b5a056f7eb72665e053d5785840475164826f8fa6c5551ae7eac531d6` was current at the time of writing. The macOS-local fingerprint is not authoritative and was not committed. |
