...
| Connector pod (Confluence)  | A synced Confluence page, comment, or policy-table answer loses recall, or its page-title citation stops resolving                               | `confluence-connector-pod` (`check:retrieval-quality`, Epic #2238)        | relevance / evidence-shape |
| Connector pod (Jira)        | A synced Jira issue description, status/type metadata line, or issue-comment answer loses recall, or its `KEY: summary` citation stops resolving | `jira-connector-pod` (`check:retrieval-quality`, Epic #2238)              | relevance / evidence-shape |
...
| Retrieval quality (scorecard) | passed | `npm run check:retrieval-quality` — workspace scorecard cases=15, local scorecard fixtures=17/17 (point-in-time snapshot as of PR #2016; later epics have grown the shared `ALL_FIXTURES` registry beyond this count — see `fixtures.ts` for the current total), threshold metadata emitted, `sealed-pod` reported `policy-denied:1`, lexical/vector/fused comparison emitted, 4 regression probes held |
