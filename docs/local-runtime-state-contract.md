...
| CLI control state         | Linux `~/.local/state/keiko/control/`; macOS `~/Library/Application Support/Keiko/control/`; Windows `%USERPROFILE%\AppData\Local\Keiko\control\`                                             | `@oscharko-dev/keiko-cli`, `@oscharko-dev/keiko-server`        | Fixed, non-overridable location for the existing activity-log sink when a CLI command must audit or remove its selected runtime-state tree. Contains body-free `logs/server.log` evidence only; never customer content or a second log format.                                                                                                                                      |
| CLI refusal state         | Linux `~/.cache/keiko/control-failures/`; macOS `~/Library/Caches/Keiko/control-failures/`; Windows `%USERPROFILE%\AppData\Local\KeikoControlFailures\`                                       | `@oscharko-dev/keiko-cli`, `@oscharko-dev/keiko-server`        | Independent, non-overridable fallback for body-free audit/uninstall refusal evidence when the primary control root overlaps the selected target or cannot be validated/opened. It is used only after proving it lies outside the protected target and uses the same activity-log format and controls.                                                                               |
...
| Workspace index | `KEIKO_WORKSPACE_INDEX_DIR` → `KEIKO_STATE_DIR/workspace-index/` → `~/.keiko/workspace-index/` |
...
| Activity logs (`logs/server.log`, including CLI control and refusal roots)                                         | yes              | yes                | deferred                                  | yes (7 days)   | n/a                                                                               | [ADR-0173](adr/ADR-0173-server-activity-log-v2-machine-reconstruction-contract.md)                      |
...
- **Uninstall `--state`** removes the manifest artifacts and then the state directory, but
  ...
  and completion records removal/retention disposition plus body-free counts. A primary-control-root
  overlap or validation/open failure is recorded through the separately isolated refusal root.
...
- `keiko audit local-state [--state-dir <path>]` — **the operator-facing command.** Without
  ...
  without persisting its path. The command refuses a target that contains the primary control state
  and records that refusal through a separately isolated failure root, preserving its physical
  read-only contract as well as its logical one.
...
- The UI database contains canonical typed and spoken chat content. ADR-0013 D10 explicitly defers
  application-level DB encryption, so regulated deployments that require chat-at-rest
  confidentiality must enforce host full-disk encryption until that decision is superseded.
