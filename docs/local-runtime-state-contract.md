...
| Lifecycle files          | `KEIKO_STATE_DIR/ui.pid`, `KEIKO_STATE_DIR/ui.meta.json`, and `KEIKO_STATE_DIR/ui.log` or default `.keiko/`                                                                  | `@oscharko-dev/keiko-cli`                                      | Runtime-only process state. `ui.meta.json` records the launched pid (plus diagnostic-only `binPath`/`startedAt`); `keiko stop` checks the recorded pid against `ui.pid` before signaling.   |
| CLI launch binary        | built checkout `./dist/cli/index.js` → `./node_modules/@oscharko-dev/keiko/dist/cli/index.js` → `KEIKO_CLI_BIN_PATH` → package-relative entry next to the running CLI module | `@oscharko-dev/keiko-cli`                                      | Which Keiko binary `keiko start` launches. The current project's install wins over an unrelated global on `PATH`. Rungs 1–2 require both the built CLI entry and the built UI static asset. |
...
| CLI launch bin  | built checkout → local `node_modules` package → `KEIKO_CLI_BIN_PATH` → package-relative fallback |
...
- `keiko start` resolves the launch binary against the current project first, so a stale global on
  `PATH` cannot win resolution. Removing an already-installed legacy global binary is still a
  one-time manual step Keiko does not perform automatically.
- `keiko stop` confirms a live pid against the identity recorded in `ui.meta.json` before signaling;
  a pid whose recorded identity no longer matches (for example after Windows pid reuse) is left
  untouched. Missing or malformed metadata falls back to the legacy pid-only behavior.
