...
- **No duplicate memory implementation** — user-facing memory remains MemoriaViva (in-desktop `memoria` window, owned by #1295; `/api/memory/*` routes and governed vault/audit/retention) plus the Conversation Center disclosure/toggle over the same BFF retrieval path. The former `/memoriaviva` route stubs were deleted in #2894 (KEIKO-0129); bookmarked deep links fall through the server's `serveStatic` fallback onto the desktop shell. The desktop Twin panel has regression coverage ensuring it does not expose a separate localStorage-backed memory store.
