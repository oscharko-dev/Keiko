...
| 7   | Verification matrix tests accurate retrieval, long-range understanding, test-time learning, selective forgetting, stale-memory, cross-scope isolation, error propagation | `tests/memory-eval/` (#215) + this document                                                        |
...
The deterministic evaluation scorecard at
[`tests/memory-eval/scorecard.json`](../tests/memory-eval/scorecard.json)
is regenerated by the eval runner (`tests/memory-eval/eval-runner.test.ts`)
on every test run. The scorecard records pass/fail for each of the eight
synthetic-fixture scenarios that exercise the full pipeline end to end.
