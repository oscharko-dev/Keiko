...
Status: Issue #862 migration evidence, refreshed during the post-#2665 audit on 2026-07-22.

## Migration decisions

- `packages/keiko-ui` uses the reviewed `next@16.2.12` and `eslint-config-next@16.2.12` baseline. The consolidated dependency review verified the aligned refresh with the UI lint, test, build, and Chromium smoke gates.
...
- The root package and UI workspace use `eslint@9.39.5`, the latest release accepted by every active Next plugin peer range. ESLint 10.8.0 was evaluated and rejected because the current `eslint-plugin-import`, `eslint-plugin-jsx-a11y`, and `eslint-plugin-react` releases still publish ESLint <=9 peer ranges; accepting npm's override makes `npm ls eslint` invalid.
