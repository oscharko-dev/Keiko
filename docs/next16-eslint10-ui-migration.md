...
- The root package and UI workspace run ESLint 10 on one installed binary. The ESLint 9 lane, the rejection of ESLint 10.7.0/10.8.0, and the evidence that superseded both are recorded under [ESLint 10 adoption](#eslint-10-adoption-issue-2777-2026-08-28).
...
- `eslint-config-next` still bundles React, import, JSX-a11y, and parser pieces whose published peer ranges do not name ESLint 10 (unchanged in `16.3.1`). The UI config therefore reuses Next's own `@next/next`, React Hooks, TypeScript, and JSX-a11y plugin objects, but filters out the React/import rule layer and avoids Next's Babel parser shim.
...
- `npm run check:dependency-hygiene` owns the single-lane invariant, at both layers, because npm
  ...
  The second check is not redundant with the first, and it is not something `npm ls` could do.
  `npm ls` raises a problem only for a missing, invalid, or extraneous edge (npm's
  `lib/commands/ls.js`, `getProblems`); a nested copy that satisfies its own workspace's declared
