...
4. **CSP alignment.** The server CSP already sets `worker-src 'self'`
   ...
   `csp.ts` ≈ line 63), so Monaco's runtime style injection needs **no** CSP change either. #1193/#1206
   must verify that no **new** `worker-src`/`script-src`/`style-src` widening is introduced for Monaco,
   and that the static export emits the worker assets into the export output so they resolve at
   `keiko ui` runtime.
5. **No-CDN proof plan.** Two checks, both owned by #1193 and re-confirmed by #1207/#1209: (a) an
   **automated** Playwright network-intercept test that loads the editor and a worker-backed feature
   and fails if any outbound request to a non-loopback origin is observed, run against both `next dev`
   (Turbopack) and the static production build; (b) a build-output assertion that the Monaco worker
   chunks are present in the static export. The automated check (a) provides regression protection, not
...
context (and ultimately to deterministic-only completion) and records the omission as evidence; it
never silently fabricates context and never blocks the editor. This mirrors the existing
...
- Test-generation workflow: `packages/keiko-workflows/src/qualityIntelligence/modelRoutedTestDesign.ts`
  ...
  gateway wiring.
- Generation port + injection scrubbing:
  `packages/keiko-server/src/qualityIntelligence/generationPort.ts` (`createQiGenerationPort`,
  `scrubEvidenceText` — NFKC normalize + strip C0/C1/invisible-format controls + neutralize
  `<qi-evidence>` delimiters). This is also the model for LLM08 retrieval-injection defence (§13).
...
| `keiko-evidence/src/connected-context-evidence.ts` (`persistConnectedContextEvidence`) + audit types                                                | Retrieval audit trail                                                       | Reuse as-is              | Editor-originated retrieval writes the same audit evidence (query, scope, files searched, excerpts, omissions).                                                           |
...
| R3  | As-you-type completion exceeds the latency/INP budget; large files degrade responsiveness                               | Performance                       | Med × Med                      | Per-keystroke uses deterministic + cheapest context only; requests are debounced (≥ 200 ms, #1207) and self-cancelling (`AbortSignal`); numeric budgets in #1207 (per-keystroke < 50 ms local work, INP ≤ 200 ms, p50/p95). Large-file resilience is quantified: beyond a threshold (e.g. > 500 KB / > 10 000 lines, #1207) the file opens read-only with completion + diagnostics disabled and a UI indicator (server already caps reads at `MAX_TEXT_PREVIEW_BYTES`, `keiko-server/src/files.ts`). |
| R3b | Monaco inflates the UI bundle / first-load JS                                                                           | Performance / supply chain        | Med × High                     | Monaco is a lazy-loaded chunk behind the `ssr: false` dynamic import and must **not** appear in first-load JS; #1207 measures the Monaco chunk gzip size, sets a documented numeric ceiling, and adds a CI assertion that fails if the chunk exceeds it or appears in the initial payload.                                                                                                                                                                                                           |
...
| R6  | Retrieved RAG context carries an injection payload into the completion/test-gen prompt (OWASP LLM08 / LLM01)            | Model boundary / prompt injection | Med × High                     | Reuse `scrubEvidenceText` (control-char strip + delimiter neutralization + `<qi-evidence>` fencing); treat retrieved chunks as untrusted; keep the injection carrier visible to the reviewer. Owner: #1211/#1206.                                                                                                                                                                                                                                                                                    |
