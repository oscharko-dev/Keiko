...
| Git delivery          | No commit, push, PR, merge, or force-push authority.   | Commit/push/PR require prompt approval; merge remains blocked.   | Commit/push/PR execute only through delivery substrate. **Merge is admitted by this same admission given a sufficient envelope** — what holds it is ADR-0087's separate approval, not this layer. Force-push is refused by the push route.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | `runBoundAuthority.test.ts` and `coding-runtime/gitOperationRequirements.test.ts` (admission plus the per-operation requirement table); `approvalStore.test.ts` (one-use approval); `pushRoutes.test.ts` (force-push refusal).                                                                            |
| Connector writes      | Write-capable scopes denied.                           | Write scopes are prompt-gated.                                   | Atlassian writes are admitted on connector scope plus action class, evaluated per mode facet with the strictest facet winning (`atlassian/actionPolicy.ts` → `decideAtlassianConnectorAction`). The envelope's `networkPolicy` is **not** consulted on that path: a deny-all envelope receives the same disposition. Network scope IS consulted on the other connector paths: `authorizeGitDelivery` (`gitDelivery/runBoundAuthority.ts` `hasRequiredScopes`, deny-all and per-scope), the sidecar tool port's `connectorAllowed` and `deliveryAllowed` (`coding-runtime/codingToolAuthorityPort.ts`), and `editorAgentAuthorizedConnectorScopes` for context reads (`editor/agentAuthorityRegistry.ts`). **No task-ref binding is enforced** — see note. | `packages/keiko-server/src/atlassian/actionPolicy.test.ts` (write admission under a deny-all network policy); `packages/keiko-server/src/coding-context/codeContextConnector.test.ts` (connector reads: scope, ceiling, content-free evidence).                                                           |
...
| Authority Envelope    | Not enough authority for mutation.                     | Prompt authority does not imply delivery autonomy.               | Required; `authorizeGitDelivery` denies unconfirmed (no accepted run), expired, workspace escape, branch escape, scope drift, and policy (mode) rejection. It takes no budget input and its denial union has no over-budget member. Budget bites one layer up: the production `GitDeliveryRunAuthorityPort` (`RuntimeAuthorityService.currentGitDeliveryAuthority`) yields no run once `revalidateRetainedRuntime` reports `authority-budget-exceeded` for elapsed `maxRuntimeMs`, which this admission then reports as `accepted-run-unavailable`. Tool-call, prompt-token, and patch-byte budgets are charged per action by the registry's `reserveUsage`, not by this admission.                                                                       | `packages/keiko-server/src/gitDelivery/runBoundAuthority.test.ts`; `packages/keiko-server/src/coding-runtime/runtimeAuthorityService.test.ts` (elapsed `maxRuntimeMs` fails retained authority closed; no pin yet drives `gitDeliveryAuthorityPort().current` past it); `codingAutonomyQaMatrix.test.ts`. |
