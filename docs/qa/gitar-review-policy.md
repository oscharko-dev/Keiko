...
| Merge blocking         | Advisory while service availability is unbounded | Findings remain actionable without making a missing vendor check a repository deadlock.                             |
| Auto-approve           | Disabled                                         | Approval is not required by branch protection and never replaces direct checks.                                     |
| `gitar-approved` label | Disabled with Auto-Approve                       | Avoids presenting advisory orchestration state as merge authority.                                                  |
| Auto-merge             | Disabled                                         | GitHub native auto-merge is governed by direct required checks.                                                     |
...
- `.gitar/rules/*.md` contains exactly five focused Pro custom checks. The governance rule enables
  ...
  direct required checks, or assign an external integration.
...
3. Request a Gitar run on the exact current head when the service is available. A new commit
...
`gitar auto-apply:off` is an incident stop command. `gitar unblock` remains prohibited. Gitar
Auto-Approve and Auto-Merge remain disabled while the integration is advisory.
