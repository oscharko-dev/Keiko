...
| Launch and provenance     | `javaProvider.test.ts` executes the governed pre-spawn contract, proves JDK 17 fails closed, pins the exact runtime/layout and final revalidation launch shape, and verifies copy-only environment, ephemeral HOME, bounded output/time, native no-egress attestation, and fail-closed provisioning behavior. |
