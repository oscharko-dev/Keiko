...
| `typescript:S3516` / `javascript:S3516` (Blocker)  | `packages/keiko-ui/public/keiko-playback-worklet.js:113`; `packages/keiko-server/src/store/relationships.ts:1079`; `packages/keiko-workflows/src/contextpack/assemble.ts:367` | 3     | (a) `AudioWorkletProcessor.process()` must return `true` from every path to keep the node alive, per the Web Audio API contract. (b)/(c) `expandFrontier`/`buildPlan` always return the _same identifier_ (`nextFrontier`/`plan`) at every exit point, but that identifier is a mutable accumulator built up differently before each return — Sonar's check compares the return _expression_, not the runtime value, so it cannot see the content differs.                                                                                                                                                                                                                                                                                                                                                         |
