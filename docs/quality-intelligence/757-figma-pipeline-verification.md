...
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------ |
| 1   | A real large board reduced to a lean snapshot; code + tests generated; no token/PII leak into snapshot or evidence.                                                                                      | Live table: boards 1/2/3 reduced 46.9% / 40.6% / 42.9%; 62 / 33 / 56 code files emitted; baseline 121 / 62 / 114 items; token leak `none` (substring scan). Snapshot no-token asserted by `figmaSnapshotBuilder.test.ts:245` ("never embeds the token anywhere in the assembled snapshot value").                                                                                                                                                                                                                                                                                                                                                                                           | PASS   |
| 2   | A single read-only PAT (from `FIGMA_ACCESS_TOKEN`), proxy-routed, authenticates the connector; never reaches the browser, snapshot, or log; runtime loads only the allowlisted PAT from `.env` at start. | Token resolved server-side at `figmaConnector.ts:114` via `resolveFigmaToken` (`figmaTokenSource.ts:32`, precedence vault > config > `FIGMA_ACCESS_TOKEN` env); materialised into `X-Figma-Token` only at the transport boundary (`figmaConnector.ts:217`); route resolves it server-side and never returns it (`figmaSnapshotRoutes.ts:576-588` passes `configToken` into the bounded snapshot build, response projection `figmaSnapshotRoutes.ts:495-502` carries no token, header note lines 14-17). Runtime `.env` load: the `keiko ui` command imports only `FIGMA_ACCESS_TOKEN` from a repo-local `.env` via `loadLocalKeikoEnv` — see note below.                                    | PASS   |
| 3   | No Figma communication after the bounded snapshot-build; all downstream stages read only the stored snapshot (snapshot = boundary).                                                                      | Figma is contacted only inside the connector + render ports (`figmaConnector.ts`, `figmaRenderPort.ts`), both invoked only during snapshot-build (`figmaResnapshot.ts`, header lines 3-10: "Figma is contacted ONLY here, within the snapshot boundary"). The QI source loader reads only the stored snapshot via the evidence store, never Figma (`figmaSnapshotAdapter.test.ts:3-4`). IR / nav / a11y / codegen are pure domain (`packages/keiko-quality-intelligence/src/domain/figma/*`, no IO).                                                                                                                                                                                        | PASS   |
| 4   | Re-snapshot is an explicit, on-demand full re-fetch (no delta/incremental); single-fetch principle holds.                                                                                                | `resnapshotFigma` (`figmaResnapshot.ts:42`) performs a deliberate FULL scoped re-fetch — "NO delta and NO incremental skip" (header lines 3-7); the connector uses scoped `GET /v1/files/:key/nodes?ids=&depth=` only, never the whole-file endpoint (`figmaConnector.ts:126`, header lines 4-5). No webhook / poll path exists.                                                                                                                                                                                                                                                                                                                                                            | PASS   |
...
| 6   | Identical behaviour on `gpt-oss-120b` (chat) + a multimodal model, capability-routed, with graceful degradation when multimodal is absent.                                                               | Capability-routed selection (no hard-coded id) is resolved by `resolveQiMultimodalSelection`, which delegates to the shared Gateway selector through `{ kind: "chat", supportsImageInput: true }` (`modelSelection.ts:148-152`); typed `{ kind: "unavailable" }` degradation to IR-only when no image-input model is configured (`modelSelection.ts:153-154`). Committed regression coverage: `modelSelection.test.ts:149-201` (unavailable when absent, by-capability selection, lowest-cost preference, and "does NOT select a vision-NAMED model lacking the capability"). The deterministic baseline stands alone (AC 5). The live run exercised the IR-only path (no multimodal call). | PASS   |
| 7   | Multimodal routing (#810) selects a vision model by capability and degrades to IR-only when absent.                                                                                                      | The QI selector calls the shared `selectConfiguredModel` path via `selectCapabilityByGatewayQuery` (`modelSelection.ts:59-69`) with `supportsImageInput: true`; capability flag `supportsImageInput` in the gateway registry (`packages/keiko-model-gateway/src/model-selection.ts:18-32`, `capabilities.ts:30-31`). Returns `unavailable` → IR-only when no model advertises it. By-capability-not-by-id pinned at `modelSelection.test.ts:195` and `matrix.test.ts:137-161`.                                                                                                                                                                                                              | PASS   |
...
| 9   | The navigation/flow graph + flow tests (#811) are produced deterministically (model-free, reproducible).                                                                                                 | `deriveNavGraph` / `deriveNavFlows` / `deriveNavTestItemsByScreen` are pure (`navGraph.ts`); byte-identical case `navGraph.test.ts:366`. Synthetic test: wired link → 1 edge + 1 navigation item, reproducible.                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | PASS   |
| 10  | The a11y baseline (#812) is produced model-free, with contrast computed from the extracted tokens.                                                                                                       | `deriveA11yTestItemsByScreen` is pure; exact WCAG contrast from IR colours (`a11yBaseline.ts`, `relativeLuminance` / `contrastRatio`); cases `a11yBaseline.test.ts:105,112,116` (≈4.48 #777/#fff, passes 21≥4.5, fails 4.48<4.5); determinism `a11yBaseline.test.ts:615`. Synthetic test: contrast / alt-text / accessible-name items fire.                                                                                                                                                                                                                                                                                                                                                 | PASS   |
...
| 12  | GOVERNANCE: no customer board content / Storybook / snapshot committed; live snapshots only in git-ignored `.keiko/`; committed tests synthetic-only.                                                    | `git check-ignore -v` resolves `.keiko/` → `.gitignore:145` and `only-for-internal-use/` → `.gitignore:170` (and nested paths). Committed test (`pipelineEndToEnd.test.ts`) uses invented synthetic fixtures only. This note records content-free metrics only.                                                                                                                                                                                                                                                                                                                                                                                                                             | PASS   |
...
- The PAT is materialised into the `X-Figma-Token` header only at the transport boundary
  (`figmaConnector.ts:217`); it is never logged, never returned in any response, and never placed in
  provenance or error bodies (`figmaConnector.ts:9-13`, `extractFigmaReason` at `figmaConnector.ts:140`
  passes only the generic `err`/`message` string to the classifier, never the token).
- The assembled Snapshot never embeds the token (`figmaSnapshotBuilder.test.ts:245`).
...
On the original closure branch (`feat/757-figma-verification`) this note and the synthetic integration
test were the only changes: no merged feature code was modified. The later audit pass (next section)
touches only this note and two test files — still no production code.

## Citation re-verification (audit against `release/0.2.0`)

This closure note is maintained as living evidence (it was last realigned to the merged code by the
release-hardening passes that followed #839). On 2026-06-15 every file:line pointer in the acceptance-
criteria table and the security check above was re-verified against the current `release/0.2.0` tip
(`977c022d`). Outcome: **all twelve #757 acceptance-criteria invariants still hold and remain backed by
committed regression tests**; the deterministic pipeline (`pipelineEndToEnd.test.ts`) passes 9/9 at this
tip. Several evidence pointers had drifted as the release evolved past #839 — design-to-code hardening
(#973, #1099), multimodal-capability routing and its single-sourced READ/WRITE mapping (#810, #762),
the determinism/reproducibility and model-independence matrix work (#763, #764), and the release gate
hardening (#1026, #1091). The drifted pointers were corrected to their current anchors so the
traceable-evidence chain resolves again; no production code or invariant was changed.

The audit also hardened one assertion in the committed synthetic test: the AC10 contrast check
previously asserted only that a `"colour contrast"` a11y item _exists_, which a bare substring match
also satisfies for an "is below WCAG AA" fail or a "needs a verifiable colour contrast" coverage
notice — so it would have survived an inverted AA threshold or broken text-colour extraction. It now
pins the computed verdict (`outcome === "pass"`, title "meets WCAG AA", exact `21:1` ratio),
RED-verified against an inverted `meetsContrastAa`. No production behaviour changed.

Finally, the audit closed a regression-coverage hole on the AC5 "NO model" invariant: the QI
domain purity guard (`purityGuard.test.ts`, ADR-0023 D4 — a pure-domain leaf with "no provider
SDKs") enforced "no IO" (`node:*` builtins + non-deterministic runtime APIs) but had no entry for
the **model boundary**, so a future source-level coupling of `@oscharko-dev/keiko-model-gateway`
into the deterministic domain — even a type-only import — would have passed every committed test. The
gateway specifier is now in `DENIED_SPECIFIER_PREFIXES` with three focused cases (value import, type
import, prose-safety), RED-verified by injecting `import type { ModelPort } from
"@oscharko-dev/keiko-model-gateway"` into `cleanToScreenIr.ts` (previously green, now flagged). The
package does not depend on the gateway, so the existing scan stays green. Test-only, zero production
behaviour change.
