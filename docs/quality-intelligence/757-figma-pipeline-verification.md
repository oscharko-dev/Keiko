...
| 2   | A single read-only PAT (from `FIGMA_ACCESS_TOKEN`), proxy-routed, authenticates the connector; never reaches the browser, snapshot, or log; runtime loads `.env` at start. | Token resolved server-side at `figmaConnector.ts:80` via `resolveFigmaToken` (`figmaTokenSource.ts:32`, precedence vault > config > `FIGMA_ACCESS_TOKEN` env); materialised into `X-Figma-Token` only at the transport boundary (`figmaConnector.ts:152`); route resolves it server-side and never returns it (`figmaSnapshotRoutes.ts:249-253`, header note line 15). Runtime `.env` load: the `keiko ui` command loads `.env` at process start via `loadLocalKeikoEnv` (`packages/keiko-cli/src/ui.ts:145`) — see note below. | PASS   |
...
**does** load a local `.env` at process start via `loadLocalKeikoEnv`
(`packages/keiko-cli/src/ui.ts:145`): it reads `<cwd>/.env`, parses `KEY=value` lines, and merges
them into the env **without overriding** values already present. So the documented run pattern
(`FIGMA_ACCESS_TOKEN` in the repo-root `.env`, then `keiko ui`) loads the PAT for the server side. The
connector still also accepts an exported env or `node --env-file`. There is no general process-wide
dotenv autoloader for non-`ui` entrypoints; callers of those must export the env or use
