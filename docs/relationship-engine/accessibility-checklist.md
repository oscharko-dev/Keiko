...
| ---------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **1.3.1**  | Info and relationships expressed programmatically.                          | Inspector sections use `<div role="heading" aria-level="3">`; edges expose a `<button>` with descriptive `aria-label` — see the `aria-label={connectionBadgeAriaLabel(...)}` usage in [`ConnectionsLayer.tsx`](../../packages/keiko-ui/src/app/components/desktop/windows/ConnectionsLayer.tsx); activity badges use `role="status"` + `aria-live="polite"` + `aria-atomic="true"`. |
...
| **1.4.13** | Content on hover or focus is dismissible, hoverable, persistent.            | Denial tooltip during creation is dismissable by `Escape`, hoverable (operator can move pointer onto the banner), and remains visible until the documented dismissal condition fires: preview denials may clear on the next valid target or inactivity timeout, while commit-phase and security-class denials persist until operator action.                                        |
...
| **2.1.2**  | No keyboard trap.                                                           | Modals trap focus and restore on close (the existing `PermControl` contract); the inspector returns focus to the originating edge `.conn-badge` on exit, or to the launching timeline row / inspector container when no originating edge exists; `Escape` releases focus mode and dismisses banners.                                                                                |
| **2.1.4**  | Character key shortcuts can be turned off or remapped.                      | Single-letter chords (`R`/`A`/`I`/`E`/`F`/`/`) are registered through the exported `useKeyboardShortcuts` function in [`useKeyboardShortcuts.ts`](../../packages/keiko-ui/src/app/components/desktop/hooks/useKeyboardShortcuts.ts) and only fire when the focus is inside the inspector or the workspace shell, never globally. They are remappable via the same registry.         |
...
| **2.4.7**  | Focus visible.                                                              | Every focusable element gets `focus-visible:outline 2px solid var(--accent)` with 2 px offset (the existing pattern in the `.arun-btn:focus-visible` and `.conn-badge:focus-visible` rules in `packages/keiko-ui/src/app/globals.css`). `focus:outline-none` without a `focus-visible:` replacement is forbidden.                                                                   |
...
| **2.5.8**  | Target size (minimum) 24×24 CSS px.                                         | Action buttons use existing `arun-btn` (≥ 28×30 from the `.arun-btn` rule in `packages/keiko-ui/src/app/globals.css`); edge `.conn-badge` is ≥ 24×24 with adequate padding (the `.conn-badge` rule in the same file); icon-only inspector buttons enforce a minimum 24×24 footprint per [#67 memory pattern](https://github.com/oscharko-dev/Keiko/issues/67).                      |
...
The keyboard map below combines the inspector chords from [inspector-spec.md "Keyboard map"](inspector-spec.md) with the surface-wide chords introduced by the blueprint. All chords are registered through the exported `useKeyboardShortcuts` function in [`useKeyboardShortcuts.ts`](../../packages/keiko-ui/src/app/components/desktop/hooks/useKeyboardShortcuts.ts); the bind-time collision check in the same file fails the build if any chord collides with `WORKSPACE_RESERVED_CHORDS` (from `@oscharko-dev/keiko-contracts`) or with the existing #66 / #67 chord set.
...
| Restore default view (clear filters, focus)              | `Escape`          | When pressed at the workspace root (no input focus), clears focus mode and dismisses the denial banner; persisted density is unchanged. |
...
6. Press `Escape` to exit inspector and restore focus to the originating edge badge, or to the launching timeline row / inspector container when no originating edge badge exists.

### Filter flow keyboard sequence

1. Press `/`. Filter input gains focus.
2. Type a filter token (e.g., `type:produces-evidence`).
3. Pause typing for 250 ms or press `Enter`. URL state updates after the documented 250 ms debounce; `aria-live="polite"` announces "Showing N of M relationships, filtered by …".
...
| Lifecycle    | `stale`           | "Stale"           | hollow square                      | inspector chip         | `--fg-muted`         |
...
| Inspector closes                                 | Focus returns to the originating edge `.conn-badge`; if there is no originating edge badge (for `?relFocus=` deep links or timeline-entry launches), focus returns to the launching timeline row when available, else to the inspector panel container.                                                                                                                                                                                                                                                                                                                                 |
...
- Auto-dismiss assertive alerts in a way that conflicts with the documented persistence rules; security-class and commit-phase denials require explicit operator dismissal.
