...
7. **Evidence**: A relationship row does not pin an evidence manifest from retention. Evidence retention defaults to independent 50-run caps for the `chat-rag` and `regulated` partitions through `maxRunsByPartition` ([`packages/keiko-contracts/src/evidence.ts`](../../packages/keiko-contracts/src/evidence.ts)); activity in one partition therefore cannot evict another partition's runs. A retired manifest surfaces as `EndpointLiveness.status: "retired"` and the relationship becomes `stale`.
