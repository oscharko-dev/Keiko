...
11. **Relationship records persisted to durable UI state MUST be limited to approved metadata only.** [ADR-0030 rule 5b](../adr/ADR-0030-workspace-security-evidence.md) is explicit that durable-state secret hardening is not yet a closed gate for `localStorage`-backed workspace state. The relationship engine therefore must not rely on a second BFF redaction barrier for browser-local persistence; only redacted metadata and references are allowed.
