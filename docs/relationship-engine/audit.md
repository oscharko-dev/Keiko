...
Historical note: this document records the Wave 1 reuse audit that preceded implementation. Where later child issues adopted a final storage, audit/activity, or UI-containment decision, the authoritative current-state documents are [ADR-0031](../adr/ADR-0031-relationship-storage-and-validation.md), [ADR-0032](../adr/ADR-0032-relationship-audit-and-activity-model.md), [ADR-0033](../adr/ADR-0033-relationship-ui-containment.md), and the later `docs/relationship-engine/*` implementation docs.
...
| `@oscharko-dev/keiko-workflows`            | `src/bug-investigation/memory.ts`, workflow-descriptor, workflow-handoff                                                                                                                                                                     |
...
3. **`node:sqlite` `--experimental-sqlite` strategy** documented in [`docs/workspace/518-architecture-blueprint.md`](../workspace/518-architecture-blueprint.md) and exercised in production by the memory vault and the existing `keiko-server` store ([`packages/keiko-server/src/store/schema.ts`](../../packages/keiko-server/src/store/schema.ts)). The same `PRAGMA user_version` schema-evolution pattern is established.
...
- No package owns a "polymorphic relationship store" today. The closest pattern is the server-owned UI persistence layer documented in [`docs/workspace/518-architecture-blueprint.md`](../workspace/518-architecture-blueprint.md) and implemented via [`packages/keiko-server/src/store/schema.ts`](../../packages/keiko-server/src/store/schema.ts) and [`packages/keiko-server/src/store/db.ts`](../../packages/keiko-server/src/store/db.ts).
...
3. **Evidence store** in [`packages/keiko-evidence/src/store.ts`](../../packages/keiko-evidence/src/store.ts) — atomic file writes, `O_EXCL`, realpath-contained, partition-aware retention via [`retention.ts`](../../packages/keiko-evidence/src/retention.ts). `DEFAULT_RETENTION` independently bounds chat/RAG and regulated evidence at 50 without evicting unknown manifests.
...
5. **A relationship referring to an evidence manifest MUST honour evidence retention.** The manifest inherits its partition and any explicit global/partition policy ([`evidence.ts`](../../packages/keiko-contracts/src/evidence.ts)). Relationships whose evidence endpoint has aged out must surface as unavailable; the API must not refetch the evidence content from any back-channel.
...
9. **Relationship store schema evolution MUST follow the `PRAGMA user_version` migration pattern** established by `keiko-memory-vault` and the existing `keiko-server` store ([`packages/keiko-memory-vault/src/schema.ts`](../../packages/keiko-memory-vault/src/schema.ts), [`packages/keiko-server/src/store/schema.ts`](../../packages/keiko-server/src/store/schema.ts)). No alternative migration tool is introduced.
10. **Relationship network surfaces MUST preserve the existing local-only transport boundary.** The product binds to loopback only ([`docs/security-and-audit-boundaries.md`](../security-and-audit-boundaries.md)); no new remote listener, no new WebSocket route, and no WebRTC surface may be introduced without a separate ADR ([`ADR-0030`](../adr/ADR-0030-workspace-security-evidence.md)).
11. **Relationship records persisted to durable UI state MUST be limited to approved metadata only.** [ADR-0030 rule 5b](../adr/ADR-0030-workspace-security-evidence.md) makes the `localStorage`-backed workspace snapshot secret-safe at the `useWorkspace` sanitizer seam. The relationship engine must still not rely on browser-local persistence as a content store or second BFF redaction barrier; only redacted metadata and approved references are allowed.
12. **Relationship summaries and evidence views MUST preserve the connected-context privacy contract.** Browser-visible summaries and persisted audit metadata may carry counts, enums, hashes, ids, and approved references, but not raw query text, raw excerpts, or unapproved path detail ([`docs/connected-context-privacy.md`](../connected-context-privacy.md)).
13. **Relationship existence MUST NOT be treated as authorization.** Issue #533's stop condition: "Stop if relationship existence is treated as sufficient authority to access models, tools, files, connectors, workflows, or evidence." Every consumer must re-check the endpoint owner's policy before acting.
14. **No new credential surface.** [ADR-0030 §"Credential handling"](../adr/ADR-0030-workspace-security-evidence.md) — credentials remain under `keiko-server` config and `keiko-memory-vault`; relationships never persist them.
...
1. **`node:sqlite` `--experimental-sqlite` flag must be applied at three sites** (CLI re-exec, vitest `execArgv`, no flag for `tsc`) — captured in [`docs/workspace/518-architecture-blueprint.md`](../workspace/518-architecture-blueprint.md) and the existing `keiko-server` / `keiko-memory-vault` store seams. New relationship-store callers inherit this constraint.
...
6. **Dependency-cruiser** (`.dependency-cruiser.cjs`, gated by `npm run arch:check` / `npm run arch:check:negative`) enforces cross-package direction rules ([ADR-0019](../adr/ADR-0019-modular-package-architecture.md) + [ADR-0020](../adr/ADR-0020-workspace-tooling-and-architecture-gate.md) D4). No custom `eslint-plugin-keiko` exists; ADR-0020 D4 explicitly rejected per-file ESLint rules for cross-package topology. A new relationship package would have to land on the existing direction matrix. The recommended layout (contract in `keiko-contracts`, store in a leaf consumer, UI in `keiko-ui`) requires no new direction rule.
7. **Static-export UI** under Next.js — dynamic routes are query-param routes. Any relationship inspector route in the UI follows this convention documented in [`docs/workspace/518-architecture-blueprint.md`](../workspace/518-architecture-blueprint.md).
8. **SSE EventSource needs `addEventListener` per event name.** A relationship-activity stream must enumerate its event names in the BFF wire types so the UI subscribes per-kind, not by string concatenation; the existing pattern is in [`packages/keiko-ui/src/lib/useSSE.ts`](../../packages/keiko-ui/src/lib/useSSE.ts).
