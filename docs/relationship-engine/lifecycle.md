...
4. **Retention inheritance.** Transitions that write into `EvidenceManifest.relationships` inherit the parent manifest's partition and applicable retention policy ([`evidence.ts`](../../packages/keiko-contracts/src/evidence.ts)). Relationship audit entries do not pin runs from eviction.
