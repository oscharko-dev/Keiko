...
The caps are enforced at the SQL barrier (every query carries `LIMIT ?`) and returned to the caller via `truncated: boolean` plus the typed `truncationReason: "max-depth" | "max-nodes" | "max-relationships" | null` on walk results, and `cycleScanTruncated: boolean` on health findings, so the UI can render an explicit truncation banner.
...
- Minimal: ≤ 5 visible edges (focused-window upper bound per blueprint)
...
| `npx vitest run packages/keiko-server/src/relationship*`                                                                                                                                                                                                                                                                                                                                            | 45 pass (5 originally re-enabled in #588; 1 additional regression added by audit follow-ups) |
