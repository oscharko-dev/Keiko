...
| 2   | A replay with a divergent body returns `relationship/idempotency-replay-mismatch` (HTTP 409). The body hash is `SHA-256` over canonicalised JSON.   | test.                                                                                                                                    |
| 3   | The idempotency cache TTL is 15 minutes and the capacity is 4096. A cache eviction does not corrupt the response of a concurrent in-flight request. | code review; the cache is process-local and synchronised.                                                                                |
| 4   | Every PATCH / DELETE rejects a missing `If-Match` with `relationship/optimistic-concurrency-required` (HTTP 428).                                   | test.                                                                                                                                    |
| 5   | An `If-Match` mismatch returns `relationship/optimistic-concurrency-conflict` (HTTP 412) with the current etag in the body.                         | test.                                                                                                                                    |
