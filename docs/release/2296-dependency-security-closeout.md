...
| `typescript-eslint`           | root                  | 8.68.0  | patch-deferred | 8.69.0 tightens `no-meaningless-void-operator`, which flags 59 existing `void <identifier>` unused-suppression idioms in tests; adopting it is its own change, not a dependency bump. |
