...
| `typescript-eslint`           | root                  | 8.69.0  | current        | Taken with the deferred work it demanded. `no-meaningless-void-operator` tightened in 8.69.0 and flagged all 59 `void <identifier>` unused-suppression idioms; each moved to the `_` marker the root ESLint config already declares, so the convention is stated once instead of in two competing forms. |
