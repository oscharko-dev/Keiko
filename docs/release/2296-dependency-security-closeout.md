...
- TypeScript-7 closeout #2270 is a different case and is **not** delivered. PR #2304's own body
  ...
  `typescript-eslint@8.69.0` independently caps the API package at `<6.1.0`. The split toolchain is
...
| `eslint`                      | root                  | 10.10.0 | current        | ESLint 10 lane; the `eslint-config-next` peer cap was cleared by #2777. The 10.10 update adds rule coverage and bug fixes without changing Keiko's configured rule set.                                                                                                                                      |
...
| `next`                        | keiko-ui              | 16.3.4  | current        | Patch on the 16.3 line taken 2026-09-09, one release after the 16.3.3 advisory fix; moved together with `eslint-config-next`.                                                                                                                                                                                |
| `eslint-config-next`          | keiko-ui              | 16.3.4  | current        | Kept exactly aligned with `next`; the two move together or not at all.                                                                                                                                                                                                                                       |
...
| `@babel/preset-env`           | root                  | 8.0.5   | current        | Dependabot #3545; reviewed Babel bug-fix rollup, including destructuring, block-scoping, Safari array bounds, and parser correctness fixes.                                                                                                                                                                  |
| `vite`                        | keiko-ui              | 8.3.0   | current        | Dependabot #3546; the workspace declaration and root override move together. Includes build/proxy performance work and fixes for CRLF code frames, package-root detection, failed-build cleanup, HTML preload targets, HMR, and asset handling.                                                              |
| `vitest`                      | root                  | 4.1.11  | major-deferred | #3474 reverts the Vitest 5 adoption for the mutation lane: Stryker 10 with Vitest 5.0.0 reports zero killed mutants despite a covering dry run, making the security mutation evidence meaningless. 4.1.11 is the patched Vitest 4 line with a clean audit result and working Stryker/Vitest instrumentation. |
| `vitest`                      | keiko-ui              | 4.1.11  | major-deferred | Kept exactly aligned with the root because workspace-local Vitest drift previously produced peer-invalid `npm ls` output. The next Vitest major move must first prove non-zero Stryker kills on the security mutation scope.                                                                                 |
| `@vitest/coverage-v8`         | root                  | 4.1.11  | major-deferred | Exact peer match for Vitest 4.1.11. Vitest 5 coverage stays deferred with the Vitest major because the security mutation runner, not the coverage package alone, is the blocking evidence path (#3474).                                                                                                      |
| `autoprefixer`                | keiko-ui              | 10.5.6  | current        | Dependabot #3547; corrects generation of legacy `-webkit-` gradients with color hints.                                                                                                                                                                                                                       |
| `@types/react`                | keiko-ui              | 19.3.0  | current        | Kept exactly aligned with React 19.3 and `@types/react-dom` across the UI and editor workspaces.                                                                                                                                                                                                             |
...
| `@types/node`                 | root                  | 26.5.1  | current        | Root declarations realigned with the UI workspace on the Node 26 line.                                                                                                                                                                                                                                       |
| `@types/node`                 | keiko-ui              | 26.5.1  | current        | Node 26 declaration refresh for the UI workspace; resolves with the root node.                                                                                                                                                                                                                               |
| `@types/react-dom`            | keiko-ui              | 19.3.0  | current        | Kept exactly aligned with React DOM 19.3 and `@types/react` across the UI and editor workspaces.                                                                                                                                                                                                             |
| `@vitejs/plugin-react`        | keiko-ui              | 6.1.1   | current        | Taken 2026-09-09; the UI build and its smoke lanes are the proof, and the baseline is unchanged.                                                                                                                                                                                                             |
| `@testing-library/react`      | keiko-ui              | 16.3.3  | current        | Fixes re-entrant `act()` behavior while dispatching events.                                                                                                                                                                                                                                                  |
| `@testing-library/user-event` | keiko-ui              | 14.6.7  | current        | Taken 2026-09-09 with the UI and editor suites as the proof; no behaviour in either depends on the deferred delta.                                                                                                                                                                                           |
| `@playwright/test`            | root                  | 1.63.0  | current        | E2E reference runner; the browser pin and Linux-authoritative D12 evidence move with it.                                                                                                                                                                                                                     |
| `prettier`                    | root                  | 3.9.6   | current        | Formatter policy is unchanged.                                                                                                                                                                                                                                                                               |
| `knip`                        | root                  | 6.35.1  | patch-deferred | Adds analysis fixes and makes plugin-config load failures fail distinctly. The later 6.36 line was not part of the reviewed Dependabot queue and remains a separate update.                                                                                                                                  |
| `corepack`                    | root                  | 0.36.0  | current        | Lockfile-verified Node 26 bootstrap for the pinned offline Yarn smoke.                                                                                                                                                                                                                                       |
...
Every action is pinned to a full 40-character commit SHA with a version comment. All 12 external
...
| `github/codeql-action`             | v4.38.0 | b96794f015dfd88f77b49b1c93e0fa7110f94c63 | current        |
| `google/osv-scanner-action`        | v2.5.1  | 6e4298ebc4db23e847df9b2e2de2939d6f066c67 | current        |
| `zizmorcore/zizmor-action`         | v0.6.4  | cc914d7f3750a2d13d75c7f184a1060aa0e9d482 | current        |

The two `patch-deferred` actions have newer upstream tags (checkout v7.0.1 and attest v4.2.2).
They remain outside this rollup because they were not part of the reviewed Dependabot queue. The
credential-bound Azure signing actions were retired by #3403 when Keiko-owned release signatures
became the mandatory cross-platform trust anchor.
...
optional `^1.0.0`) and 1.0.9 under `keiko-local-knowledge`, carrying eleven redundant platform
...
- **Resolved dependency versions**: `vitest` and `@vitest/coverage-v8` move to 4.1.11 and
  `@napi-rs/canvas` is deduplicated to 1.0.9 through a root override. Reverting restores the
