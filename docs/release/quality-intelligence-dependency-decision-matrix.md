...
| typescript         | (top-level) | deterministic TS/JS language service (completion, diagnostics, hover, symbols) for the Keiko Editor (#1198); also the build-time compiler | approved-runtime | Apache-2.0   | platform-foundation | promoted dev→runtime by #1198: the governed, server-side, model-free, offline deterministic language service (ADR-0042 D4). Pure-JS, no native addon, no install hook, no network. Declared by keiko-server and promoted into the staged root dependency graph because npm local-directory dependencies require root provisioning (ADR-0021 D1). | medium     | in-browser Monaco ts.worker / monaco-languageclient — rejected by ADR-0042 D4 as ungoverned (bypasses audit + model-boundary governance)                                                             |
