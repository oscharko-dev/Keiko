...
| pg                 | (top-level) | PostgreSQL client for the separately deployed governed feedback-intake service (exact `pg@8.22.0`)                                        | approved-runtime | MIT        | platform-foundation | Required by #2074 / ADR-0125 for durable hosted intake, transaction isolation, retention, and key-deletion evidence. Exact-pinned in the workspace manifest and lockfile (`sha512-8wih1vVIBMxoUM2oB4soJsD9tDnDpLv4OXBJ+EJzFsvycD+lfyIreC2gGHq78f8jbLLt+bvlPTFdFZfJkOuzAA==`). The installed pure-JS closure is MIT/ISC, has no lifecycle hook or native addon; optional pure-JS `pg-cloudflare@1.4.0` ships, while optional peer `pg-native` is neither declared nor installed. `npm audit --omit=dev` reported 0 vulnerabilities on 2026-07-11. | medium     | Hand-rolling the PostgreSQL wire/TLS/authentication protocol was rejected as higher security and maintenance risk; volatile in-memory storage is test-only and cannot satisfy ADR-0125 durability. |
