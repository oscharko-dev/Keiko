...
7. **Completeness (fail-closed on unapproved deps).** Every external dependency that ships in the published `@oscharko-dev/keiko` runtime graph — the `dependencies`/`optionalDependencies` of the root manifest and of every workspace in the source runtime inventory — maps to an `approved-runtime` decision-matrix row. Only validated runtime workspace packages governed by the `@oscharko-dev/*` vendoring contract are exempt; runtime `@types/*` packages are external dependencies and require an approved decision-matrix row like any other shipped package. A new permissively-licensed runtime dependency added without a matrix row fails the gate.
