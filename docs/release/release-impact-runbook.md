...
- `github-issue-comment:<owner>/<repo>#<issue>#<comment>` — the referenced comment must exist on the referenced issue in the current GitHub repository, be authored by an allowed release owner, and contain the literal phrase `Approved-for-publish: <package-name>@<version>` for the exact root package version being published. This form exists because GitHub refuses self-approval of one's own pull requests, which a solo release owner can never satisfy; it carries the same intent — a durable, GitHub-verified owner approval artifact — at the same strictness (owner decision, 2026-08-08).
