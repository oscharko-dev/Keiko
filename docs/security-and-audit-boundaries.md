...
| Command execution     | Verification and tool execution route through `@oscharko-dev/keiko-tools` terminal-policy allowlists. Arbitrary shell execution is not an approved UI or workspace surface. External-source shell input is bounded by the deterministic-`dev` delivery envelope (ADR-0135) and the coding-task authority-envelope model (ADR-0141).                                                                                                                             |
...
- Voice capture uses Realtime WebRTC for a media-only, no-memory-tool-access transport
  scoped by ADR-0154 (Canonical Twin Voice pipeline); no other WebRTC surface is approved.
