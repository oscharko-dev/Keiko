...
| **Existing controls to relax** | Re-justify, do not silently drop: `Permissions-Policy ... microphone=()` (`keiko-server/src/headers.ts` line 13) must be scoped to the voice origin before `getUserMedia`; CSP `default-src 'none'` / `connect-src 'self'` (`keiko-server/src/csp.ts` lines 61, 65) must be extended for any browser-direct media or STUN/TURN (proxied-SDP minimizes this). |
