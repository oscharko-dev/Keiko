...
| Graph decision defines node, edge, layout, selection, grouping, navigation, fit-to-view sufficient for implementation                          | [ADR-0026 §"Graph substrate per-term coverage"](../adr/ADR-0026-workspace-substrate.md#graph-substrate-per-term-coverage) — `node`, `edge`, `fit-to-view`, `selection` present in existing surfaces; `layout` is a fixed linear pipeline (not configurable); `grouping` and intra-graph `navigation` are deferred to a follow-up ADR           |
