...
Audit date: 2026-06-06. Audit basis: repository HEAD `d834195d` on integration branch `dev`.
...
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `@oscharko-dev/keiko-ui`                   | `app/layout.tsx`, `app/page.tsx`, `app/components/desktop/**` (35+ components, hooks, windows registry, modals, contexts, widgets), `app/local-knowledge/**`, `app/memory/**`, `app/quality-intelligence/**`, `lib/**` |
...
| Concept area                                                                     | Existing Keiko surface                                                                                                                                               | Disposition                                                                                                   |
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Object taxonomy                                                                  | `WindowType` enum in `windows/WindowsRegistry.ts` (19 types)                                                                                                         | EXTEND — promote existing window types into a documented product taxonomy; do not invent parallel terminology |
...
| Concept area                                                  | Existing Keiko surface                                                                                                   | Disposition                                                                                                                                      |
| ------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------ |
| Command model & palette                                       | `modals/CommandPalette.tsx` (Command type, build/select/run), `AppShell.buildCommands`                                   | REUSE — UX blueprint formalizes the existing command vocabulary, adds discoverability rules and shortcut conflict rules where not yet documented |
...
| Error/empty/loading/blocked states                            | `AlertBanner` in connector-graph, `AgentGateCard.tsx` for review-needed, `lk-alert`/`lk-status` classes in `globals.css` | REUSE — UX blueprint documents these patterns as the standard                                                                                    |
...
| Panels and inspectors                                                     | `widgets/panels/**` (10 panels)                                                                                                                               | REUSE                                                   |
...
| Empty / loading / streaming / blocked / degraded / error states           | Existing patterns in `local-knowledge/connector-graph.tsx`, `memory/components`, `quality-intelligence/QualityIntelligencePanel.tsx`, `lk-status`, `lk-alert` | REUSE                                                   |
| Responsive behavior                                                       | Existing CSS variables, breakpoints in `globals.css`, PWA viewport hooks                                                                                      | REUSE                                                   |
| Focus ring, hit target, contrast rules                                    | WCAG 2.5.8-compliant `lk-btn-primary`, focus-visible rules, contrast guidance recorded in PR #71 / ADR-0014                                                   | REUSE — UI blueprint documents the existing rules       |
...
| Global navigation and project context                                                                                   | `Header.tsx` (workspace tabs), `LeftRail.tsx` (PRIMARY + SECONDARY rail tools), `ConnectedScopePill.tsx` | REUSE — extend the LeftRail + Header to surface the explicit workspace `Project` context if the audit identifies a gap |
...
| Empty / loading / blocked / degraded / error shell states                                                               | `lk-status` and `lk-alert` classes, `AgentGateCard` for blocked, `NotificationsPanel` for degraded       | REUSE — verify each shell-level state is reachable; close any gap with a single AlertBanner addition                   |
| Keyboard-reachable navigation and focus rings                                                                           | Existing `rail-btn` + `hd-tool` ARIA + focus-visible rules                                               | REUSE                                                                                                                  |
| Shell-level status indicators (connected project, model availability, workflow readiness, verification/evidence access) | `Footer.tsx`, `ConnectedScopePill.tsx`, gateway status in `useGateway` hook                              | REUSE — verify each indicator is reachable; close gap with a small `ShellStatusIndicators` addition if needed          |
...
| Undo/redo boundary                        | Not implemented today                                                        | NEW — implement an undo stack scoped to UI-state actions (window move, window resize, window close-from-list, palette command application that is purely UI). The substrate must REFUSE to rewrite evidence, executed commands, applied patches, verification records, and model-call records. This refusal is enforced as a typed boundary in the action contract, not as a runtime check. |
...
- A typed undo/redo action contract and a small undo stack hook for UI-state actions. The redo/undo refuses by type to mutate evidence/patch/verification/model-call surfaces.
- A keyboard matrix implementation backing the minimum shortcut set from #523.
...
| Object descriptors (title, icon, default size, config schema, render fn, tool/singleton flag) | `WindowTypeDef` interface in `WindowsRegistry.ts`                                                                                               | EXTEND — add lifecycle, trust boundary, authority requirement, persistence expectation fields to the descriptor; these are new fields, not a new registry |
| Identity for workspace objects                                                                | Existing window `id` (string), `AppWindow.id`                                                                                                   | REUSE                                                                                                                                                     |
| Persistence boundary                                                                          | UI hooks own transient state; `node:sqlite` UI persistence (#62) owns durable layout; `keiko-evidence` owns evidence; `keiko-workspace` owns FS | REUSE — separate durable / transient / evidence references at the descriptor level                                                                        |
...
- Extended descriptor fields on `WindowTypeDef` and a registry helper that validates them at registration time.
- A small persistence-boundary classifier per descriptor (`durable` vs `transient` vs `evidence-reference`).
- A small test that confirms descriptors cannot register persistence of secrets, raw evidence content, or token-bearing artifacts.
...
| Fit-to-view / reset-view                             | `useWorkspace` exposes camera state                                                                                                              | REUSE                                                |
...
| Performance guardrails                               | Existing window count tested in `useWorkspace.ts` / `AppShell.test.tsx`                                                                          | REUSE — performance evidence already exists          |
...
| Supply-chain verification | `package.json` manifests; lockfile; `arch:check` rules in `eslint-plugin-keiko`                                                                           | REUSE — verify no new runtime/devDep introduced by Wave 4       |
...
| Extended `WindowTypeDef` descriptor fields: `lifecycle?`, `trustBoundary?`, `authority?`, `persistence?`, plus a small descriptor validator | `packages/keiko-ui/src/app/components/desktop/windows/WindowsRegistry.ts` and a sibling validator file     | Single registry file plus its validator + tests |
| Typed undo/redo action contract, undo stack hook, refusal-by-type for evidence/patch/verification/model-call mutations                      | `packages/keiko-ui/src/app/components/desktop/hooks/useUndoStack.ts` (new) and a single contract type file | One hook, one contract, no new package          |
| Minimum keyboard-shortcut matrix wiring (per #523)                                                                                          | `packages/keiko-ui/src/app/components/desktop/hooks/useKeyboardShortcuts.ts` (new)                         | One hook                                        |
...
Total new TypeScript implementation code introduced by Epic #518 is therefore bounded to: one extended descriptor interface, one descriptor validator, one undo-stack hook with its contract type, one keyboard-shortcut hook, and the test files that cover them. No new package. No new dependency. No new persistence store.
