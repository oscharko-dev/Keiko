...
- `npx vitest run src/app/components/desktop/windows/descriptor-meta.test.ts src/app/components/desktop/widgets/panels/InspectorPanel.test.tsx src/app/components/desktop/hooks/workspace-persistence.test.ts` (run from `packages/keiko-ui`) — 3 files, 23 tests passing.
...
| 5 (No raw secrets)        | Follow-up [#598](https://github.com/oscharko-dev/Keiko/pull/598) materially tightened the browser-local workspace snapshot: transient windows are dropped, `evidence-reference` windows persist only declared reference fields, and `durable.config` payloads are stripped before browser-local write or restore. Issue [#600](https://github.com/oscharko-dev/Keiko/issues/600) closes the remaining `useWorkspace` browser-local secret-shape gap by reusing the existing secret-shape detector, adding local checks for browser-relevant credential classes, redacting secret-shaped `durable.ui` free text to a fixed sentinel, and dropping unsafe reference values. This proves the browser-local workspace layout snapshot is secret-safe at the `useWorkspace` seam; it does not claim universal coverage for unrelated durable stores owned by other packages. | PASS   |
...
Rules 1-5 remain green for the workspace foundation scope. Issue [#600](https://github.com/oscharko-dev/Keiko/issues/600) closes the previously open browser-local `useWorkspace` snapshot residual risk by ensuring secret-shaped workspace config values are redacted or omitted before `localStorage` persistence and again during restore.
...
- `npx vitest run src/app/components/desktop/windows/descriptor-meta.test.ts src/app/components/desktop/widgets/panels/InspectorPanel.test.tsx src/app/components/desktop/hooks/workspace-persistence.test.ts` (run from `packages/keiko-ui`) — 3 files, 23 tests passing, including the `review` evidence-binding assertion and the browser-local persistence sanitization checks.

### Outcome

PASS for the current `origin/dev` state. Evidence-bearing objects remain classified as `evidence-reference`, and the browser-local workspace snapshot is descriptor-aware and secret-safe for the `useWorkspace` persistence seam. That still should not be mistaken for a universal secret-redaction or durable-state proof beyond the workspace layout snapshot.
...
| --------------------------------------------------------------------------------------------------------------- | -------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Browser-local workspace persistence previously lacked secret-shaped config filtering beyond descriptor metadata | Medium   | Closed by issue [#600](https://github.com/oscharko-dev/Keiko/issues/600) for the `useWorkspace` browser-local snapshot; unrelated durable stores remain governed by their owner-package seams. |
