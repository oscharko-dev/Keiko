...
| Object type                    | `WindowType`                | Owner                                                             | Lifecycle states                                                    | Trust boundary                                                 | User authority requirement                               | Persistence                                                           | Initial delivery    |
| ------------------------------ | --------------------------- | ----------------------------------------------------------------- | ------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------- | --------------------------------------------------------------------- | ------------------- |
| **Repository context**         | `project`, `files`          | `keiko-workspace` + `keiko-ui`                                    | `none → connecting → connected → degraded → disconnected`           | Path containment + denied paths                                | User must connect a project root                         | Durable (project registry); FS access transient per request           | Shipped (Epic #518) |
| **Conversation**               | `chat`                      | `keiko-ui` + `keiko-server` chat handlers + `keiko-model-gateway` | `draft → streaming → final → archived`                              | Model Gateway routes all model calls; redactor scrubs payloads | User may send; agent may propose; apply requires confirm | Durable (UI persistence #62)                                          | Shipped (Epic #518) |
| **Workflow run**               | `agents`, `integ`           | `keiko-workflows` + `keiko-server` run handlers                   | `proposed → running → blocked → needs review → verified → archived` | Workflow descriptor + tool boundary                            | User explicitly starts a run                             | Durable (run ledger)                                                  | Shipped (Epic #518) |
| **Generated patch / proposal** | `review`                    | `keiko-tools` applyPatch + `keiko-ui` `ReviewWidget`              | `proposed → reviewing → applied → reverted → archived`              | applyPatch validator; workspace path containment               | User explicitly applies or rejects                       | Durable for proposals (evidence reference); applied changes go to FS  | Shipped (Epic #518) |
| **Verification result**        | `review` (verification tab) | `keiko-verification` + `keiko-server` run handlers                | `pending → running → passed → failed → cancelled`                   | Verification orchestrator                                      | User triggers; agent may propose                         | Durable (run ledger entry)                                            | Shipped (Epic #518) |
| **Evidence artifact**          | `review` (evidence tab)     | `keiko-evidence`                                                  | `created → redacted → archived`                                     | Redacted-by-construction                                       | Read-only in UI                                          | Durable (evidence store)                                              | Shipped (Epic #518) |
| **Terminal session**           | `terminal`                  | `keiko-tools` terminal policy + `keiko-server` terminal routes    | `idle → running → completed → blocked → cancelled`                  | Command allow-list + path containment                          | User confirms each command class                         | Transient (session)                                                   | Shipped (Epic #518) |
| **Browser tab**                | `browser`                   | `keiko-server` browser policy                                     | `closed → connecting → live → error`                                | Allow-list policy                                              | User opens explicitly                                    | Transient                                                             | Shipped (Epic #518) |
| **Editor view**                | `editor`                    | `keiko-ui` editor widget                                          | `closed → reading → editing → unsaved → saved`                      | Workspace path containment                                     | User opens / edits                                       | Transient unless file save                                            | Shipped (Epic #518) |
| **Settings**                   | `settings`                  | `keiko-ui` settings panel + `keiko-server` config                 | `viewing → editing → saved`                                         | Config validators                                              | User-only mutation                                       | Durable (config store)                                                | Shipped (Epic #518) |
| **Notifications**              | `notifications`             | `keiko-ui` notifications panel                                    | `unread → read → dismissed`                                         | UI-only                                                        | User dismisses                                           | Transient                                                             | Shipped (Epic #518) |
| **Activity / Timeline**        | `activity`                  | `keiko-ui` timeline panel                                         | `live → archived`                                                   | UI-only                                                        | UI-only                                                  | Transient                                                             | Shipped (Epic #518) |
| **Resources**                  | `resources`                 | `keiko-ui` resources panel                                        | `live`                                                              | UI-only                                                        | UI-only                                                  | Transient                                                             | Shipped (Epic #518) |
| **Inspector**                  | `inspector`                 | `keiko-ui` inspector panel                                        | `empty → focused`                                                   | UI-only                                                        | UI-only                                                  | Transient                                                             | Shipped (Epic #518) |
| **Mobile pairing**             | `mobile`                    | `keiko-server` host check + `keiko-ui` mobile panel               | `paired → unpaired`                                                 | Host check + authentication                                    | User confirms                                            | Durable (pairing token in OS keychain only — never persisted to repo) | Shipped (Epic #518) |
| **Search**                     | `search`                    | `keiko-ui` + `keiko-workspace` repoSearch                         | `idle → searching → results → error`                                | Workspace path containment                                     | UI-only                                                  | Transient                                                             | Shipped (Epic #518) |
| **Plugins**                    | `plugins`                   | `keiko-ui` plugins panel                                          | `idle → installed → disabled`                                       | Registry-only (no runtime hosting in #518)                     | UI-only                                                  | Durable (config)                                                      | Shipped (Epic #518) |
| **Automations**                | `automations`               | `keiko-ui` automations panel                                      | `idle → enabled → disabled`                                         | Workflow descriptors only                                      | User-only                                                | Durable (config)                                                      | Shipped (Epic #518) |
| **Keiko Twin**                 | `keiko`                     | `keiko-ui` Twin panel + `keiko-memory-*`                          | `live`                                                              | Memory governance                                              | User-only                                                | Durable (memory vault)                                                | Shipped (Epic #518) |
| **Connector**                  | `connector`                 | `keiko-ui` connector picker + `keiko-local-knowledge`             | `connecting → connected → degraded → disconnected → error`          | workspace / Local Knowledge — ui, fs                           | User selects a capsule or capsule-set                    | fs-reference (declared capsule binding in UI config)                  | Shipped (Epic #189) |

These 20 first-class types correspond 1:1 with the existing `WindowType` enum. The product boundary is therefore "the workspace foundation must support exactly these object types, with the lifecycle/trust/authority/persistence fields documented above." The `connector` type was added by Epic #189 after the initial #518 delivery.
...
| **Server behavior**         | `keiko-server` request handlers; `keiko-workspace` path validators | `keiko-server` containment, workspace validation, discovery         | No — constraint enforcement is inline; workspace invariants are not recorded as separate ledger events |
...
These were proposed in addition to the five ADRs already named by #520. Both have since been delivered as part of Epic #518:

6. **ADR — Workspace authority model** — now delivered as [ADR-0030: Workspace security, evidence, and trust boundaries](../adr/ADR-0030-workspace-security-evidence.md). ADR-0030 operationalizes the five inviolable rules in this document and adds compile-time enforcement.
7. **ADR — Extension-ready object contracts** — now delivered as [ADR-0029: Workspace object registry and extension contract](../adr/ADR-0029-workspace-object-registry.md). ADR-0029 defines the six-step extension protocol, the `WorkspaceDescriptorMeta` sidecar, and the closed-enum registry contract.

No further ADR work is required for these candidates.
