...
| --------- | ---------------- | -------------------------------------------------------------------------------------- |
| Header    | `Header.tsx`     | Full width; fixed height (`--header-h`); always visible                                |
| LeftRail  | `LeftRail.tsx`   | Fixed narrow column (`--rail-w`); always visible; collapsible per memory #63           |
...
| Footer    | `Footer.tsx`     | Full width; fixed height; always visible                                               |
...
| **Evidence artifact** (`review/evidence`)       | Read-only manifest renderer with download / export                  | Evidence icon, redacted-only badge |
...
| **Loading**                | Skeleton blocks; `aria-busy`; no spinning until > 500ms                                                                               | `lk-status` skeleton                   |
...
| **Error**                  | `role="alert"` + `aria-live="assertive"`; explanation of what failed, what wasn't changed, what to try; retry CTA only when retryable | `lk-alert` error variant               |
| **Offline / disconnected** | Footer offline indicator; affected widgets show offline copy with reconnect CTA                                                       | Footer status                          |
| **Stale**                  | Stale badge on the affected row; refresh CTA                                                                                          | `lk-status` stale variant              |
...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------- | --------------------------------------------------------------- |
| Color tokens (`--bg`, `--fg`, `--accent`, `--ink`, `--ink-inverse`, `--ink-muted`, `--ink-dim`, `--danger`, `--warning`, `--success`, `--bg-elevated`, `--border`) | `globals.css`                                                           | No new color tokens added by Wave 4                             |
| Theme system                                                                                                                                                       | `useTheme.ts`, `data-theme="light"/"dark"`, `colorScheme: "dark light"` | No new theme                                                    |
| Contrast: `ink-inverse #1a1e23` on accent `#4EBA87` = 6.94:1 (PASS)                                                                                                | Memory pattern #63                                                      | Do NOT use `ink #ffffff` on `accent #4EBA87` (2.41:1 FORBIDDEN) |
| Contrast: `ink-dim #6b7280` = 3.25:1 on chrome — only for incidental disabled-control labels                                                                       | Memory pattern #63                                                      | Not for informational tooltip text; use `ink-muted` (≥4.5:1)    |
...
| Layout supports the primary journeys                                                                         | Layout regions ↔ journey table (UX blueprint § Primary journeys)                                                   |
