...
| **Loading**                | Skeleton blocks; `aria-busy`; no spinning until > 500ms                                                                                       | `.rv-loading` / `.rv-skel` for skeleton blocks; `.lk-loading` for dimmed-text label                                                   |
...
| **Blocked**                | Warning border + blocking explanation + suggested action                                                                                      | `lk-alert` (danger-styled error variant); a dedicated block variant is to be created if a visually distinct blocked state is required |
| **Warning**                | Amber border + warning copy                                                                                                                   | `lk-alert` (danger-styled error) + `--warn` border token for amber cue; a dedicated warning variant is to be created if required      |
...
| **Stale**                  | Inline stale badge on the affected row; refresh CTA; `aria-live="polite"` on state change                                                     | Reuse Footer status pattern + `lk-alert` (with `--warn` left-border token) where the row is also blocked                              |
