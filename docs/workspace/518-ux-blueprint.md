...
| Region                                                           | Owner             | Keyboard reach (forward contract for #527)             | Behavior                                                                                                                                        |
| ---------------------------------------------------------------- | ----------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| **LeftRail**                                                     | `AppShell`        | `Alt+1..7` by visible rail order (to be wired by #527) | Tabs to global tools (Project, Search, Plugins, Automations, Keiko mobile) and to the Keiko Twin (top of rail)                                  |
| **Header tabs**                                                  | `Header`          | `Ctrl+1..9` cycles tabs (to be wired by #527)          | Switches workspace tabs (multi-workspace context)                                                                                               |
...
- Lists commands generated by `buildAppShellCommands` in `AppShell.tsx` plus contextual commands from focused windows.
- Filters by fuzzy match across command id, label, and category.
- `Enter` runs; `Esc` closes; arrow keys move.

### Workspace tabs

- The Header tab list shows the current active workspace context. Multiple tabs allow switching between project contexts.
...
- Browser-reserved shortcuts (`Cmd/Ctrl+T` open tab, `Cmd/Ctrl+R` reload, `Cmd/Ctrl+W` close tab, etc.) are never claimed by the workspace — `WORKSPACE_RESERVED_CHORDS` in `keiko-contracts` is the source of truth and the substrate throws on binding any reserved chord. Closing a focused window is therefore done via the window close button or the command palette (`window.close` command), not a keyboard chord.
...
```
// Rect = WorkspaceUiRect; View = WorkspaceUiView; SelectionState = WorkspaceUiSelectionState
type Action =
  | { kind: "ui.window.move"; before: WorkspaceUiRect; after: WorkspaceUiRect; windowId: string }
  | { kind: "ui.window.resize"; before: WorkspaceUiRect; after: WorkspaceUiRect; windowId: string }
