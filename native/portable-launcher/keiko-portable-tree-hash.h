...
static int keiko_tree_hash_posix_fd(int root_descriptor, uint64_t deadline_ms,
                                    char output[65]) {
  int root = dup(root_descriptor);
  ...
  size_t index;
  int hash_initialized = 0, result = 0;
  if (root == -1 || !keiko_tree_collect(root, &before, &before_snapshots, deadline_ms))
    goto cleanup;
  if (!keiko_sha256_init(&tree)) goto cleanup;
  hash_initialized = 1;
  if (!keiko_sha256_update(&tree, "KHT1", 4)) goto cleanup;
  ...
  result = 1;
cleanup:
  if (hash_initialized) keiko_sha256_clear(&tree);
...
static int keiko_tree_hash_posix(const char *root_path, uint64_t deadline_ms,
  ...
  int result;
  if (root == -1) return 0;
  result = keiko_tree_hash_posix_fd(root, deadline_ms, output);
  close(root);
  return result;
}
