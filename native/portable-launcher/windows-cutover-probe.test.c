...
static int rename_pending(const keiko_paths *paths) {
  ...
  if (source == INVALID_HANDLE_VALUE || existing == INVALID_HANDLE_VALUE || directory == INVALID_HANDLE_VALUE ||
      ...
      existing_fact.size.QuadPart <= 0 || source_fact.id.VolumeSerialNumber != directory_id.VolumeSerialNumber ||
