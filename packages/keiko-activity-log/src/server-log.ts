...
function correlationIdOrUnknown(value: string | undefined): string {
  return value !== undefined && isActivityLogCorrelationId(value)
    ? value
    : ACTIVITY_LOG_UNKNOWN_CORRELATION_ID;
}
...
const ACTIVITY_LOG_WRITER_REJECTED_OPERATION = defineActivityLogOperation({
  contractKind: "activity-log-operation",
  schemaVersion: 1,
...
function applyEnvelopeFields(record: Record<string, unknown>, event: ServerLogEvent): void {
  ...
  if (
    ...
    isActivityLogCorrelationId(event.parentCorrelationId)
  ) {
...
const WRITER_GRAPH_TOKEN = Object.freeze({});
...
export class ActivityLogWriterOwnershipError extends Error {
  public constructor() {
    super("The process Activity Log writer is already owned by another module graph.");
    this.name = "ActivityLogWriterOwnershipError";
  }
}
...
export function claimActivityLogWriterOwnership(stateDir: string): void {
  ...
  if (existingValue === undefined) {
    const owner: ProcessWriterOwner = {
      graphToken: WRITER_GRAPH_TOKEN,
      reject: emitWriterOwnershipRejection,
    };
    Reflect.set(globalThis, ACTIVITY_LOG_WRITER_OWNER_KEY, owner);
    return;
  }
...
export function createFileServerLogSink(
  ...
  // Claim before mkdir, policy resolution, recovery, retention or any other filesystem mutation.
  // The Symbol.for slot spans source and built copies loaded into the same Node process. Ownership
  // is deliberately never released: closing a sink ends a segment, not this process lifetime.
...
export function appendDurableServerLogBatch(
  ...
  // Claim before the level fast path and outside the destination-safety catch. The latter is a
  // closed filesystem result, whereas duplicate writer ownership is a process invariant that must
  // surface as ActivityLogWriterOwnershipError.
  claimActivityLogWriterOwnership(stateDir);
