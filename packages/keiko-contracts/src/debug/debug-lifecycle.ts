...
const EVIDENCE_KEY_COUNT = Object.keys(EVIDENCE_KEYS).length;
...
function hasExactKeys(record: Record<string, unknown>): boolean {
  ...
  return (
    names.length === EVIDENCE_KEY_COUNT &&
    enumerableNames.length === EVIDENCE_KEY_COUNT &&
    Object.getOwnPropertySymbols(record).length === 0 &&
    names.every((key) => Object.hasOwn(EVIDENCE_KEYS, key)) &&
    enumerableNames.every((key) => Object.hasOwn(EVIDENCE_KEYS, key))
  );
}
