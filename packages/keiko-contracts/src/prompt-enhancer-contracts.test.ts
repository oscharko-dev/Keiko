...
const sampleEnhancedPrompt: EnhancedPrompt = {
  ...
  groundingPlan: {
    strategy: "hybrid",
    required: true,
    allowedRetrievalModes: ["hybrid-fusion", "supplied-context"],
    sourcePriority: [
      { source: "supplied-context", priority: 1, required: true },
      { source: "model-parametric-knowledge", priority: 2, required: false },
    ],
    citation: { discipline: "require-citations", granularity: "per-claim" },
    recency: { volatile: false, requireAsOfDate: false, flagPotentiallyStale: false },
    contradictionPolicy: "synthesize-with-caveats",
    noAnswerConditions: ["insufficient-evidence", "contradictory-evidence"],
    directives: [
    ...
    ragEvaluation: [
      {
        ...
        instruction: "Every claim must be entailed by the cited evidence.",
      },
    ],
  ...
  it("an enhanced prompt cannot encode tool, secret, or egress authority as a field", () => {
    ...
    expect([...keys].sort()).toEqual(
      [
        ...
        "taskdecomposition",
        "uncertaintyhandling",
        // Grounding plan (Issue #1311): a closed, provider-neutral source policy. None of these keys
        // encode a tool/secret/egress/authority grant — the plan is data, not a capability token.
        "groundingplan",
        "strategy",
        "required",
