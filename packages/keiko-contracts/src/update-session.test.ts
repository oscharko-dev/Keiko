...
describe("update session portable contract", () => {
  ...
  it("models redacted portable install and asset summaries", () => {
    ...
    expect(UPDATE_PORTABLE_SIDECAR_VERIFICATION_STATUSES).toEqual(["verified", "failed"]);
    expect(UPDATE_PORTABLE_SIDECAR_FAILURE_CODES).toContain("sidecar-digest-mismatch");
  ...
  it("models content-free portable staging summaries", () => {
    const sidecar: UpdatePortableSidecarSummary = {
      name: "opencode-compatible",
      kind: "coding-runtime",
      upstreamName: "OpenCode-compatible",
      ...
      payloadSha256Prefix: "e".repeat(12),
      sizeBytes: 1234,
      status: "verified",
    };
    ...
    expect(stage.sidecarRuntimes?.[0]).not.toHaveProperty("payloadRootPath");
    expect(stage.sidecarRuntimes?.[0]).not.toHaveProperty("executablePath");
  });
