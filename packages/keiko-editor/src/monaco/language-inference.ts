...
/** Monaco built-in language ids the Keiko Editor infers for local tokenisation (monaco-editor 0.55.1). */
...
/** A Monaco language id the editor can render. */
 ...
 *
 * TSX/JSX intentionally fold onto the `typescript`/`javascript` ids: monaco-editor 0.55.1 has no
 * distinct `tsx`/`jsx` language id — its TypeScript/JavaScript grammars tokenise JSX in `.tsx`/`.jsx`
 * files — so AC coverage of "TSX" and "JSX" means the extensions resolve, not that new ids exist.
 */
export const MONACO_LANGUAGE_BY_EXTENSION: Readonly<Record<string, MonacoLanguageId>> = {
  // TypeScript family (incl. TSX)
  ts: "typescript",
  mts: "typescript",
  cts: "typescript",
  tsx: "typescript",
  // JavaScript family (incl. JSX)
  js: "javascript",
  mjs: "javascript",
  cjs: "javascript",
  jsx: "javascript",
  ...
  yaml: "yaml",
  yml: "yaml",
  // Other first-class source languages
  py: "python",
  pyi: "python",
  java: "java",
  go: "go",
