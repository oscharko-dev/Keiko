...
function reserveBudget(
  ...
  if (reservation === undefined) throw new TypeError("Legacy-port catalog budget denied");
  if (!context.budgetPort.check(reservation, context))
    throw new TypeError("Legacy-port catalog budget revoked before dispatch");
  return reservation;
}
...
async function dispatch(
  ...
  const reservation = reserveBudget(context, descriptor, invocationId);
