...
describe("createDefaultProviderRuntimeRegistry", () => {
  ...
  it("keeps local-session productive dispatch fail-closed until the runtime bridge lands", () => {
    const registry = createDefaultProviderRuntimeRegistry();
    expect(() =>
      registry.resolve("codex-chat", localSessionProvider(), {
        requestId: "fixed-id",
        costClass: "medium",
        now: () => 0,
      }),
    ...
    ).toThrow(/provider type 'openai-codex-local-session'/);
  });
});
