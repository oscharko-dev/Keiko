...
describe("redact", () => {
  ...
  it("redacts repeated PEM begin headers through one end marker as one bounded block", () => {
    const begin = "-----BEGIN RSA " + "PRIVATE KEY-----";
    const end = "-----END RSA " + "PRIVATE KEY-----";
    const input = `${Array.from({ length: 5_000 }, () => begin).join("\n")}\n${end}`;

    const result = redactWithRuleCounts(input);

    expect(result.value).toBe("[REDACTED]");
    expect(result.rules).toContainEqual({ code: "credential-shape", count: 1 });
  });

  it("redacts from an unmatched PEM begin marker through the end of the input", () => {
    const begin = "-----BEGIN RSA " + "PRIVATE KEY-----";
    const body = "MIIEvQIBADAN-sensitive-key-body";

    const result = redactWithRuleCounts(`prefix\n${begin}\n${body}`);
  ...
  it("counts each overlapping assignment and URL secret only once", () => {
    const result = redactWithRuleCounts(
      [
        "X-API-Key: opaque-key-value",
        "postgres://user:password@db.internal/app",
        "https://opaque-token@host/repo",
      ].join("\n"),
    );

    expect(result.rules).toContainEqual({ code: "credential-assignment", count: 1 });
    expect(result.rules).toContainEqual({ code: "url-userinfo", count: 2 });
  });
