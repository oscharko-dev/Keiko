...
function confirmationMatches(request: AutonomousDeliveryPolicyRequest): boolean {
  return (
    ...
    request.confirmation.approvalProofDigest === request.authorityEnvelope.approvalProofDigest
  );
}
...
function denialReasonFor(
  ...
  return (
    validateAuthority(request.authorityEnvelope) ??
    lifecycleDenialReason(request) ??
    requirementDenialReason(request)
  );
}
