...
interface ParsedAutonomousDeliveryConfirmRequest {
  readonly authorityEnvelope: CodingWorkbenchAuthorityEnvelope;
}

interface AutonomousDeliveryExecutionPolicyContext {
...
const CONFIRM_TOP_LEVEL_KEYS: ReadonlySet<string> = new Set(["schemaVersion", "authorityEnvelope"]);
...
const DEFAULT_AUTONOMOUS_DELIVERY_APPROVAL_STORE = createAutonomousDeliveryApprovalStore();
...
function validateConfirmRequest(
  input: unknown,
  deps: UiHandlerDeps,
): ParsedAutonomousDeliveryConfirmRequest | RouteResult {
  if (!isPlainObject(input) || !hasOnlyAllowedKeys(input, CONFIRM_TOP_LEVEL_KEYS)) {
    return errResult(400, "AUTONOMOUS_DELIVERY_BAD_REQUEST");
  }
  if (scanForbiddenStrings(input)) return errResult(400, "AUTONOMOUS_DELIVERY_FORBIDDEN_PAYLOAD");
  if (scanUnsafeFormatChars(input) || input.schemaVersion !== "1") {
  ...
  const parsed = validateCodingWorkbenchAuthorityEnvelope(input.authorityEnvelope);
  if (!parsed.ok || !deploymentCeilingVerified(parsed.value, deps)) {
    return errResult(400, "AUTONOMOUS_DELIVERY_BAD_REQUEST");
  }
  return { authorityEnvelope: parsed.value };
}
...
function approvalStore(deps: UiHandlerDeps): AutonomousDeliveryApprovalStore {
  return deps.autonomousDeliveryApprovalStore ?? DEFAULT_AUTONOMOUS_DELIVERY_APPROVAL_STORE;
}

function serverDeploymentCeiling(deps: UiHandlerDeps): CodingWorkbenchMode {
  return deps.autonomousDeliveryDeploymentCeiling ?? "governed-assist";
}

function deploymentCeilingVerified(
  envelope: CodingWorkbenchAuthorityEnvelope,
  deps: UiHandlerDeps,
): boolean {
  const ceiling = serverDeploymentCeiling(deps);
  return (
    envelope.deploymentCeiling === ceiling &&
    envelope.effectiveMode === resolveEffectiveCodingWorkbenchMode(envelope.requestedMode, ceiling)
  );
}

function executionPolicyContext(
  parsed: ParsedAutonomousDeliveryRequest,
  deps: UiHandlerDeps,
): AutonomousDeliveryExecutionPolicyContext {
  const nowIso = new Date().toISOString();
  const serverStopped = deps.autonomousDeliveryStopState?.isStopped(parsed.authorityEnvelope.runId);
  return {
    approvalProofVerified: approvalStore(deps).consume(
      parsed.authorityEnvelope,
      parsed.confirmation,
      nowIso,
    ),
    deploymentCeilingVerified: deploymentCeilingVerified(parsed.authorityEnvelope, deps),
    operatorStopped: parsed.operatorStopped === true || serverStopped === true,
  };
}
...
function delegateContext(
  ...
  return {
    ...
    // The delegated git route logs under the ORIGINATING request's id, not a fresh one, so an
    // autonomous delivery's git failures stay joinable to the delivery (AGENTS.md §8 Rule 1).
...
async function runAutonomousDelivery(
  ...
  const policyContext = executionPolicyContext(parsed, deps);
  for (const [index, step] of parsed.operations.entries()) {
    const result = await runStep(
      parsed,
      policyContext,
      step,
      parsed.operations.length - index,
      ctx,
      deps,
    );
    steps.push(result);
...
async function runStep(
  parsed: ParsedAutonomousDeliveryRequest,
  policyContext: AutonomousDeliveryExecutionPolicyContext,
  ...
  const decision = decideAutonomousDeliveryOperation({
    ...
    approvalProofVerified: policyContext.approvalProofVerified,
    deploymentCeilingVerified: policyContext.deploymentCeilingVerified,
    step,
    usage: parsed.usage,
    nowIso: new Date().toISOString(),
    remainingStepCount,
    operatorStopped: policyContext.operatorStopped,
  });
  ...
  if (step.kind === "connector-operation") {
    return runConnectorStep(parsed, policyContext, step, decision.evidence, deps);
  }
...
async function runConnectorStep(
  ...
  if (connector === undefined) {
    const decision = autonomousDeliveryDeniedStep(
      connectorPolicyRequest(parsed, policyContext, step),
...
export async function handleAutonomousDeliveryConfirm(
  ctx: RouteContext,
  deps: UiHandlerDeps,
): Promise<RouteResult> {
  const read = await readParsed(ctx.req);
  if (!read.ok) return read.result;
  const parsed = validateConfirmRequest(read.value, deps);
  if ("status" in parsed && "body" in parsed) return parsed;
  ...
  return {
    ...
    body: {
      schemaVersion: "1",
      runId: parsed.authorityEnvelope.runId,
      confirmation: approvalStore(deps).issue(parsed.authorityEnvelope, confirmedAt),
    },
  };
}
...
export const AUTONOMOUS_DELIVERY_ROUTE_GROUP: readonly RouteDefinition[] = [
  {
    method: "POST",
    pattern: "/api/coding-workbench/autonomous-delivery/confirm",
    handler: handleAutonomousDeliveryConfirm,
  },
  {
    method: "POST",
