...
2. Discover: keiko_workspace_discover returns only bounded, allowed workspace-relative file paths matching a short query. Use it when the task does not already identify the files; use * only for a bounded repository overview. keiko_repository_search searches repository text with bounded, scope-checked results and continuation cursors; use its matches to locate the file and then read the relevant window before editing.
