...
// allowed without a bound digest. Any other request line — a non-root path, a non-empty query, or
// both — is admitted only when the grant bound a request-line digest and the decoded pathname plus
// visible query hashes to it; otherwise the request is a path/query-smuggling attempt and fails
// closed. Only the INITIAL request is bound this way — redirect hops are site-controlled, not
