// #2276 audit finding (low, epic-wide): the Python/Pyright provider had no dedicated
// performance-evidence coverage for workspace load, warm-operation reuse, diagnostics delay,
// resource hygiene, large-file handling, and disposal — those dimensions were only exercised
// indirectly via shared LSP-infra tests (lspProcessManager.test.ts, managedLspControl.test.ts)
// that never spawn through the Python provider spec. This is a hermetic, in-process measurement
// (no real pyright-langserver, no network): it proves the mechanism — one warm pooled process
// reused across ops, bounded latency, no leaked in-flight requests, a large document served
// without hanging or respawning, and a graceful shutdown/exit sequence on disposal — rather than
...
beforeEach(() => {
  binDir = mkdtempSync(join(tmpdir(), "keiko-pyright-perf-bin-"));
  root = mkdtempSync(join(tmpdir(), "keiko-pyright-perf-ws-"));
  mainPyPath = join(root, "main.py");
  writeFileSync(join(binDir, "pyright-langserver"), "#!/bin/sh\n", { mode: 0o755 });
  chmodSync(join(binDir, "pyright-langserver"), 0o755);
  writeFileSync(mainPyPath, "def value():\n    return 1\n", "utf8");
});
...
function diagnosticsRequest(text: string): LanguageServiceRequest {
  return {
    ...
    root,
    document: { path: "main.py", languageId: "python", text },
  };
}
...
function countingSpawn(): {
  ...
  const spawn: LspSpawnFn = () => {
    ...
    const controller = createFakeLspProcess({
      ...
      results: providerConformanceResults(mainPyPath, false),
    });
...
function runOp(
  ...
  const rules: readonly CommandRule[] = [{ executable: "pyright-langserver" }];
  return runHostLanguageOperation(request, {
    workspace: workspaceInfo(),
    processEnv: { PATH: binDir },
    commandRules: rules,
    overlayAbsolutePath: mainPyPath,
...
function largePythonSource(functionCount: number): string {
  const lines: string[] = [];
  for (let index = 0; index < functionCount; index += 1) {
    lines.push(`def generated_fn_${String(index)}():`, `    return ${String(index)}`);
  }
  return `${lines.join("\n")}\n`;
}

describe("Pyright provider performance evidence (#2276)", () => {
  it("workspace load: the first Python operation spawns pyright-langserver exactly once and completes", async () => {
    const { spawn, spawnCount } = countingSpawn();
    const startedAtMs = performance.now();

    const outcome = await runOp(diagnosticsRequest("def value():\n    return 1\n"), spawn);
  ...
  it("large-file degradation: a large Python source document completes on the warm process without respawning", async () => {
    const { spawn, spawnCount } = countingSpawn();
    await runOp(diagnosticsRequest("def value():\n    return 1\n"), spawn);
    const large = largePythonSource(20_000);
  ...
  it("disposal: shutdown gracefully sequences shutdown/exit and a later op spawns a fresh process", async () => {
    ...
    // A well-behaved pyright-langserver exits on the "exit" notification before the grace-period
