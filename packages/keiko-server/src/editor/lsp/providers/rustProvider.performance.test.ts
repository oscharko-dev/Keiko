// #2279 audit finding (medium, epic-wide pattern from #2275/#2277/#2278): the rust-analyzer
// provider had no dedicated performance-evidence coverage for cargo metadata/index cold start,
// warm-operation reuse, large-crate handling, and disposal, despite the issue's Expected
// Verification explicitly requiring it. Mirrors goProvider.performance.test.ts,
// javaProvider.performance.test.ts, and shellProvider.performance.test.ts. This is a hermetic,
// in-process measurement (no real rust-analyzer, no cargo, no network): it proves the mechanism —
// one warm pooled process reused across ops, bounded latency, a large generated crate source
...
// prepareSpawn is overridden (like the Java performance test) because RUST_PROVIDER_SPEC.prepareSpawn
// is prepareRustSpawn, an egress-isolation wrapper via planIsolatedRun/bwrap that would otherwise
// throw "Rust LSP egress isolation unavailable" for every operation in this hermetic environment.
...
beforeEach(() => {
  binDir = mkdtempSync(join(tmpdir(), "keiko-rust-perf-bin-"));
  root = mkdtempSync(join(tmpdir(), "keiko-rust-perf-ws-"));
  libRsPath = join(root, "lib.rs");
  for (const name of ["cargo", "rust-analyzer", "rustc", "rustfmt"]) {
    writeFileSync(join(binDir, name), "#!/bin/sh\n", { mode: 0o755 });
    chmodSync(join(binDir, name), 0o755);
  }
  writeFileSync(join(root, "Cargo.toml"), "[package]\nname='app'\nversion='0.1.0'\n", "utf8");
  writeFileSync(libRsPath, "pub fn value() -> u8 { 1 }\n", "utf8");
});
...
function diagnosticsRequest(text: string): LanguageServiceRequest {
  return {
    ...
    root,
    document: { path: "lib.rs", languageId: "rust", text },
  };
}
...
function countingSpawn(): {
  ...
  const spawn: LspSpawnFn = () => {
    ...
    const controller = createFakeLspProcess({
      ...
      results: providerConformanceResults(libRsPath, true),
    });
...
function largeRustSource(functionCount: number): string {
  const lines: string[] = [];
  for (let index = 0; index < functionCount; index += 1) {
    lines.push(`pub fn generated_fn_${String(index)}() -> u32 { ${String(index)} }`);
  }
  return `${lines.join("\n")}\n`;
}

describe("rust-analyzer provider performance evidence (#2279)", () => {
  it("cargo metadata/index cold start: the first Rust operation spawns rust-analyzer exactly once and completes", async () => {
    const { spawn, spawnCount } = countingSpawn();
    const startedAtMs = performance.now();

    const outcome = await runOp(diagnosticsRequest("pub fn value() -> u8 { 1 }\n"), spawn);
  ...
  it("index latency: the health-snapshot latency histogram records bounded warm latency", async () => {
  ...
  it("large crates: a large generated Rust source file completes on the warm process without respawning", async () => {
    const { spawn, spawnCount } = countingSpawn();
    await runOp(diagnosticsRequest("pub fn value() -> u8 { 1 }\n"), spawn);
    const large = largeRustSource(20_000);
  ...
  it("disposal: shutdown gracefully sequences shutdown/exit and a later op spawns a fresh process", async () => {
    ...
    // A well-behaved rust-analyzer exits on the "exit" notification before the grace-period
    // SIGKILL escalation fires: dispose() unconditionally sends SIGTERM as belt-and-suspenders,
    // but must never need to escalate to SIGKILL here (that path is reserved for unresponsive
    // servers).
