...
// V37 (#3565 Observation 17): admits `model-unavailable` and `workspace-unqualified`, the codes a
// refused run start answers with instead of the generic `authority-resolution-failed`. A refused
// start never persists a snapshot, but the contract admits the literals and the co-located
// `codingRuntimeSnapshotStore` round-trip over `CODING_WORKBENCH_RUNTIME_FAILURE_CODES` pins that
// the CHECK admits every one of them; it was red on the two added literals before this migration.
//
// Mechanically identical to V33: SQLite cannot ALTER a table-level CHECK, so the table is rebuilt
// with its current shape — the V33 columns plus V34 `pause_reason` and V36 `issue_purpose`, whose
// own CHECKs are carried over unchanged. No column is added or removed, so this needs no
// `legacySchemaTestFixture.ts` rollback fragment, and no sibling table is touched.
const V37_SQL = `
CREATE TABLE coding_runtime_snapshots_v37 (
  run_id TEXT NOT NULL PRIMARY KEY,
  schema_version TEXT NOT NULL,
  state TEXT NOT NULL,
  revision INTEGER NOT NULL,
  ...
  pause_reason TEXT
    CHECK (pause_reason IS NULL OR pause_reason IN ('workspace-script-trust')),
  issue_purpose TEXT NOT NULL DEFAULT 'delivery'
    CHECK (issue_purpose IN ('context', 'delivery')),
  CHECK (
    schema_version = '1'
    AND state IN ('starting','ready','running','paused','awaiting-approval','stopping','succeeded','failed','cancelled','taken-over','recovery-required')
...
INSERT INTO coding_runtime_snapshots_v37 (
  run_id, schema_version, state, revision, requested_mode, runtime_source, model_source,
  failure_code, created_at, updated_at, terminal_at, recovery_acknowledged_at,
  predecessor_run_id, task_digest, workspace_digest, operator_digest, authority_digest,
  binding_digest, provenance_digest, tool_call_count, patch_byte_count, model_request_count,
...
ALTER TABLE coding_runtime_snapshots_v37 RENAME TO coding_runtime_snapshots;
