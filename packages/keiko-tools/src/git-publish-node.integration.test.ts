...
describe("governed remote publish (Node) — AC5", () => {
  ...
  it(
    "classifies a real non-fast-forward rejection at execution time",
    async () => {
  ...
  it(
    "classifies a real protected-ref rejection (pre-receive hook declined) at execution time",
    async () => {
  ...
  it(
    "blocks a force push by policy and never touches the remote (AC4)",
    async () => {
      await governedPush(pushCommand({ setUpstreamTracking: true }));
  ...
  it(
    "blocks a push to a protected/shared target the safe pack does not permit (AC2)",
    async () => {
      await governedPush(pushCommand({ setUpstreamTracking: true }));
