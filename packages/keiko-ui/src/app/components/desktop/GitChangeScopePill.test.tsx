...
describe("GitChangeScopePill — description apply affordance (#3400 final-audit F5)", () => {
  ...
  function prScope(overrides: Partial<ChatGitChangeScope> = {}): ChatGitChangeScope {
    return makeGitChangeScope({
      pullRequestNumber: 1499,
  ...
  it("approves and applies the exact proposal generated by the Chat turn", async () => {
  ...
  it("does not let an older apply response mask a newer stale scope", async () => {
    ...
    await user.click(screen.getByTestId("git-change-description-approve"));
    await user.click(screen.getByTestId("git-change-description-apply"));
    await waitFor(() => expect(applyDescription).toHaveBeenCalledOnce());

    const stale = prScope({ snapshotDigest: "f".repeat(64), descriptionStatus: "stale" });
    rerender(
      <GitChangeScopePill
        chat={makeChat({ gitChangeScopes: [stale] })}
        updateScopes={vi.fn()}
        refreshScope={vi.fn()}
        approveDescription={approveDescription}
        applyDescription={applyDescription}
  ...
  it("disables apply for a stale connected scope, even after the proposal was approved", async () => {
    // Approval succeeds while current, then a refresh flips the scope stale before Apply
  ...
  it("renders a blocked apply outcome's state and reason code body-free", async () => {
    const chat = makeChat({ gitChangeScopes: [prScope()] });
    const approveDescription = vi.fn().mockResolvedValue({
      schemaVersion: "1",
      proposalId: "prop-1",
      expiresAt: "x",
    });
    const applyDescription = vi.fn().mockResolvedValue({
      outcome: "blocked",
