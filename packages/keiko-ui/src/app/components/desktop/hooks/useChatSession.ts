...
export function useChatSession(options: UseChatSessionOptions = {}): UseChatSessionResult {
  ...
  // Images are excluded from extraction (they stay on the metadata-only attachments path). A
  // document with no retained File (synthetic fixture) is skipped. Read failures surface a fixed,
  // path-safe alert and never abort the send. Returns the wire entries to attach plus a
  // disclosure list.
  //
  ...
  const buildDocumentContext = useCallback(async (): Promise<{
    ...
    const undeliverable = undeliverableAttachmentNotices(pendingAttachments);
