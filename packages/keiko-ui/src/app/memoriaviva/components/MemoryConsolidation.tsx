...
// Extracted from the MemoryConsolidation return JSX (SonarCloud S3776 cleanup):
// each nested ternary/conditional block below used to live inline in the
// component's return statement. Pulling them into named render helpers (same
// pattern as ReviewAction / StaleFlagEntry / SummaryStatusNotice above) keeps
// each piece independently simple and removes the nested decision tree from
// the component function itself. Pure extraction — no branch/condition changed.
function ConsolidationHeaderNav({
  onBack,
  t,
}: {
  readonly onBack?: (() => void) | undefined;
...
interface ConsolidationSettingsFormProps {
  readonly settings: StartMemoryConsolidationInput;
...
function ConsolidationSettingsForm({
  ...
  onCancel,
  t,
}: ConsolidationSettingsFormProps): ReactNode {
  return (
    <section
    ...
    >
      <form onSubmit={onSubmit} style={{ display: "grid", gap: 16 }}>
        ...
        >
          ...
          <SettingsField
            ...
            onChange={(fieldName, fieldValue) => {
              onChangeSetting(fieldName, fieldValue * DAY_MS);
            }}
          />
          <SettingsField
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "var(--space-5)" }}>
          ...
          >
            {submitting ? t("memoria.consolidation.starting") : t("memoria.consolidation.start")}
          </button>
          <button
            type="button"
            className="lk-btn lk-btn-ghost"
...
function JobStatsGrid({
  jobRecord,
  activeJob,
  t,
}: {
  readonly jobRecord: MemoryConsolidationJobEnvelope | null;
  readonly activeJob: MemoryConsolidationJob;
  readonly t: I18nTranslate;
}): ReactNode {
  return (
    <div
    ...
    >
      ...
      <div>
        ...
        <div style={{ fontVariantNumeric: "tabular-nums" }}>
          ...
          {activeJob.result?.truncated === true ? t("memoria.consolidation.truncated") : ""}
        </div>
      </div>
      <div>
        <div style={{ color: "var(--fg-muted)", fontSize: 12 }}>
        ...
        <div style={{ fontVariantNumeric: "tabular-nums" }}>{jobRecord?.memoryCount ?? 0}</div>
      </div>
    </div>
  );
}

function JobErrorAlerts({
  jobError,
  activeJobError,
}: {
  readonly jobError: string | null;
  readonly activeJobError: string | undefined;
}): ReactNode {
  return (
    <>
      {jobError !== null ? (
      ...
      {activeJobError !== undefined && activeJobError.length > 0 ? (
        <div role="alert" className="lk-alert">
          {activeJobError}
        </div>
      ) : null}
...
function ConsolidationSummaryBadges({
  summary,
  t,
}: {
...
interface ConsolidationJobStatusProps {
  readonly jobRecord: MemoryConsolidationJobEnvelope | null;
  readonly activeJob: MemoryConsolidationJob | null;
  readonly jobError: string | null;
  readonly summary: ConsolidationSummary | null;
  readonly t: I18nTranslate;
}

function ConsolidationJobStatus({
  jobRecord,
  activeJob,
  jobError,
  summary,
  t,
}: ConsolidationJobStatusProps): ReactNode {
  return (
    <section
    ...
    >
      {activeJob === null ? (
        <p style={{ margin: 0, color: "var(--fg-muted)" }}>{t("memoria.consolidation.noJob")}</p>
      ) : (
        <>
          <JobStatsGrid jobRecord={jobRecord} activeJob={activeJob} t={t} />

          <JobErrorAlerts jobError={jobError} activeJobError={activeJob.error} />

          {summary !== null ? <ConsolidationSummaryBadges summary={summary} t={t} /> : null}
...
interface ReviewItemsPanelProps {
  readonly items: readonly MemoryConsolidationReviewItem[];
...
function ProposedEdgesPanel({
  edges,
  onOpenDetail,
  t,
}: {
  readonly edges: MemoryConsolidationResult["edgesProposed"];
  readonly onOpenDetail?: ((id: string) => void) | undefined;
  readonly t: I18nTranslate;
}): ReactNode {
  return (
    ...
    >
      ...
      {edges.length === 0 ? (
        <p style={{ margin: 0, color: "var(--fg-muted)" }}>{t("memoria.consolidation.noEdges")}</p>
      ) : (
        <ul
          style={{
            listStyle: "none",
...
interface ConsolidationResultsProps {
  readonly result: MemoryConsolidationResult | undefined;
  readonly visibleReviewItems: readonly MemoryConsolidationReviewItem[];
  readonly resolvingConflictId: string | null;
  readonly resolveErrorsById: Partial<Record<string, string>>;
  readonly onOpenDetail?: ((id: string) => void) | undefined;
  readonly onResolveConflict: (item: MemoryConsolidationReviewItem) => void;
...
function ConsolidationResults({
  ...
  return (
    ...
    >
      ...
      <ProposedEdgesPanel edges={result.edgesProposed} onOpenDetail={onOpenDetail} t={t} />
    </section>
  );
}
  ...
  return (
    <>
      <header className="lk-header mc-consolidation-header">
        ...
        <ConsolidationHeaderNav onBack={onBack} t={t} />
      </header>
