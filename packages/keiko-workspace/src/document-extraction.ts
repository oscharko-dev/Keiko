...
function decodeTextForExtraction(
  bytes: Uint8Array,
  allowIncompleteTail: boolean,
): StepResult<{ readonly text: string }> {
  // fatal decoding is the second binary-classification gate after the byte probe. UTF-16 source
  // is accepted only when BOM/pattern sniffing identifies it as text; arbitrary invalid bytes still
  // surface as a binary-file failure instead of replacement-character prompt content.
  const decoded = decodeTextBytes(bytes, undefined, { allowIncompleteTail });
  return decoded === undefined ? binary() : { step: "ok", text: decoded.text };
}
...
async function readAndCap(
  ...
  // When the byte slice was capped below the file size a codepoint may straddle the boundary.
  // The shared decoder backs off incomplete UTF-8 sequences and odd UTF-16 byte counts so a clean
  // text file is not misclassified as binary because the budget ended mid-character.
  const decoded = decodeTextForExtraction(read.bytes, read.bytes.length < file.size);
  if (!isStepOk(decoded)) {
    return decoded;
  }
