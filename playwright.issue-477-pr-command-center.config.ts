...
// playwright.issue-476-git-publish.config.ts: build the packaged CLI, boot the real UI server, run a
...
const stateId = process.env.GITHUB_RUN_ID ?? `issue-477-pr-command-center-${String(process.pid)}`;
...
export default defineConfig({
  testDir: "tests/e2e",
  testMatch: "pr-command-center-477.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 180_000,
  expect: {
  ...
  projects: [
    {
      name: "chromium",
      // A tall viewport so the maximized pull-request window's controls are within the page — the
      // desktop window is fixed-positioned, so vertical room must come from the viewport itself.
