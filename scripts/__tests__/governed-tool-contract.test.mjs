...
describe("architecture phase fixtures (not production evidence)", () => {
  it.each(["commit-uncertain", "release-uncertain"])(
    "accepts only a truthful failed accounting uncertainty %s",
    (budgetDisposition) => {
      const value = {
      ...
      expect(validateEvidenceExample(original, "terminal", value)).toEqual([]);
      for (const patch of [
        { status: "completed", reason: "none" },
        { effectStarted: !value.effectStarted },
        { reservationId: null },
      ])
        expect(validateEvidenceExample(original, "terminal", { ...value, ...patch })).not.toEqual(
          [],
        );
    },
  );

  it("represents unresolved tool identity only in a rejection before reservation or effects", () => {
    const rejection = {
      ...phaseExample("terminal"),
      toolRef: null,
      status: "invalid",
    ...
    expect(validateEvidenceExample(original, "terminal", rejection)).toEqual([]);
    for (const patch of [
      { status: "completed", reason: "none" },
      ...
      expect(validateEvidenceExample(original, "terminal", { ...rejection, ...patch })).not.toEqual(
        [],
      );
    for (const phase of Object.keys(original.phases).filter((phase) => phase !== "terminal"))
      expect(
        validateEvidenceExample(original, phase, { ...phaseExample(phase), toolRef: null }),
      ).not.toEqual([]);
  });
