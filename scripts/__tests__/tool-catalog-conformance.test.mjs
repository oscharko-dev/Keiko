...
describe("initial compiler and finite migration conformance gate", () => {
  ...
  // for how long the real work below may take before vitest gives up on it. That work has grown
  // with the catalog: under the coverage-instrumented scripts lane it measured 28,408 ms on a green
  // run (36007956425, PR #3600) and 30,039 ms on run 36073404876 (PR #3602), so the 30 s budget
  // had no margin left and a loaded runner turned a passing gate red. The budget below is sized to
  // the measured cost with the same ~4x headroom the 15 s -> 30 s raise gave a ~7 s test.
