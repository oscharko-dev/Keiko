...
describe("workflow Node toolchain parity", () => {
  it("finds the Node setup steps at all, so a parsing regression cannot pass this file vacuously", () => {
    ...
    // plan and tag lanes and the stable build's publish-request lane. Epic #3495 (#3498) retired
    ...
    // release.yml is the authoritative dispatch. Issue #3519 split the Windows cross-platform
    // proof into its own lane. Total: 29 lanes.
