  ...
  while (pending.length > 0) {
    ...
    for (const name of readdirSync(directory).sort().reverse()) {
      ...
      if (stat.isDirectory()) pending.push(path);
        ...
        files.push({ path, relativePath: relative(root, path).replaceAll("\\", "/"), size: stat.size });
        if (files.length > MAX_TOOLCHAIN_FILES) {
          throw new Error("toolchain tree exceeds the bounded file-count limit");
        }
      } else throw new Error("toolchain trees may contain only directories and regular files");
    }
  }
...
export function inspectVerifierToolchain({
  compilerPath,
  referenceDirectory,
}) {
...
function compileVerifier({ compilerPath, referenceDirectory, source, scratchRoot, spawn = spawnSync }) {
...
export function renderGeneratedVerifierAsset({ assembly, source, toolchain }) {
  ...
  return `// Generated by scripts/generate-windows-portable-authenticode-verifier.mjs. Do not edit.\n` +
...
function parseArguments(argv) {
  ...
  for (let index = 2; index < argv.length; index += 1) {
    ...
    else if (key === "--compiler") options.compilerPath = value;
    ...
    if (key !== "--inspect") index += 1;
  }
  if (
    options.compilerPath === undefined ||
    options.referenceDirectory === undefined
  ) {
