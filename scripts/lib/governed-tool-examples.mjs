...
function validField(contract, key, value) {
  if (key === "toolRef" && value === null) return true;
...
function validEnumField(contract, key, value) {
  ...
  if (key === "budgetDisposition") return TOOL_BUDGET_DISPOSITIONS.includes(value);
...
function phaseProblems(contract, phase, value) {
  const errors = [];
  if (value.toolRef === null && !unresolvedToolRejection(phase, value))
    errors.push("unresolved tool identity outside pre-reservation rejection");
  if (phase === "terminal") {
...
function unresolvedToolRejection(phase, value) {
  return (
    phase === "terminal" &&
