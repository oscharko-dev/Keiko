...
function releaseNotes(rootPackage, options) {
  return [
    `Automated Keiko ${rootPackage.version} release.`,
    "",
    `- Published ${rootPackage.spec} to ${options.registry}.`,
    `- npm dist-tag: ${options.tag}.`,
    "- Registry install smoke passed before this GitHub Release was created or updated.",
    "- Release generated by `npm run release:publish`.",
  ].join("\n");
}
...
function ensureGithubRelease(rootPackage, options) {
  ...
  const notes = releaseNotes(rootPackage, options);
...
try {
  ...
  ensureGithubRelease(rootPackage, options);
