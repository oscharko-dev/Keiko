...
sign_file() {
  ...
  if [[ "$role" == "system-extension-manager" ]]; then entitlements="native/runtime-supervisor/macos/system-extension/system-extension-install.entitlements.plist"; fi
  if [[ "$role" == "endpoint-security-extension" ]]; then entitlements="native/runtime-supervisor/macos/system-extension/endpoint-security.entitlements.plist"; fi
...
if [[ "$phase" == "provisional" ]]; then
  ...
  while IFS= read -r relative; do
    [[ -z "$relative" ]] && continue
    entitlements="native/portable-launcher/macos-entitlements.plist"
    if [[ "$relative" == *.systemextension ]]; then entitlements="native/runtime-supervisor/macos/system-extension/endpoint-security.entitlements.plist"; fi
    codesign --force --options runtime --timestamp --keychain "$KEIKO_KEYCHAIN_PATH" \
  ...
  node scripts/macos-portable-signing.mjs prepare-qualified-payload --stage-root "$stage" --target "$target"
else
  node scripts/macos-portable-signing.mjs validate-runtime-qualification \
    --stage-root "$stage" --target "$target"
fi
codesign --force --options runtime --timestamp --keychain "$KEIKO_KEYCHAIN_PATH" \
  --entitlements native/runtime-supervisor/macos/system-extension/system-extension-install.entitlements.plist \
  --sign "$APPLE_DEVELOPER_ID_IDENTITY" "$app" >/dev/null 2>&1

notarize_app() {
  local submission="$scratch/notarization.zip" result="$scratch/notary-result.json"
...
verify_signed_path() {
  ...
  elif [[ "$role" == "system-extension-manager" || "$role" == "host-app" ]]; then
    node -e 'const fs=require("fs"),cp=require("child_process"); const p=process.argv[1],j=`${p}.json`; cp.execFileSync("plutil",["-convert","json","-o",j,p]); const x=JSON.parse(fs.readFileSync(j)); if(JSON.stringify(x)!==JSON.stringify({"com.apple.developer.system-extension.install":true})) process.exit(1)' "$entitlements"
  elif [[ "$role" == "endpoint-security-extension" ]]; then
    node -e 'const fs=require("fs"),cp=require("child_process"); const p=process.argv[1],j=`${p}.json`; cp.execFileSync("plutil",["-convert","json","-o",j,p]); const x=JSON.parse(fs.readFileSync(j)); if(JSON.stringify(x)!==JSON.stringify({"com.apple.developer.endpoint-security.client":true})) process.exit(1)' "$entitlements"
  else
...
while IFS= read -r relative; do
  [[ -z "$relative" ]] && continue
  role="default"
  if [[ "$relative" == *.systemextension ]]; then role="endpoint-security-extension"; fi
  verify_signed_path "$payload/$relative" "$role"
done < <(node -e 'const x=require(process.argv[1]); for(const e of x.nestedBundles) console.log(e)' "$inventory")
verify_signed_path "$app" host-app
