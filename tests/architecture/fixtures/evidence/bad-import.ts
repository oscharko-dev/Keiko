 ...
 *
 * Deliberately violates the strict evidence variant of ADR-0019 §"Required
 * Dependency Direction" rule 3 (`keiko-evidence` may depend only on
 * `keiko-contracts`, `keiko-security`, and `keiko-workspace`). Exists only
 * to prove the architecture gate fires on real violations for the evidence
 * boundary.
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3d-evidence-only-contracts-security-workspace`
 *     in `.dependency-cruiser.cjs` extends its `from.path` regex to include
 *     `tests/architecture/fixtures/evidence/` so this file is treated as if
 *     it were under the `keiko-evidence` boundary, and its `to.path` regex
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (evidence boundary)";
