 ...
 *
 * Deliberately violates the memory-capture strict variant of ADR-0019
 * §"Required Dependency Direction" rule 3 (`keiko-memory-capture` may
 ...
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3g-memory-capture-only-contracts-security`
 *     in `.dependency-cruiser.cjs` extends its `from.path` regex to
 *     include `tests/architecture/fixtures/memory-capture/` so this file
 *     is treated as if it were under the `keiko-memory-capture` boundary,
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (memory-capture boundary)";
