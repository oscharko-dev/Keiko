 ...
 *
 * Deliberately violates the memory-consolidation strict variant of
 * ADR-0019 §"Required Dependency Direction" rule 3 (`keiko-memory-
 * consolidation` may depend only on `keiko-contracts` and
 * `keiko-security`). Exists only to prove the architecture gate fires
 * on real violations for the memory-consolidation boundary.
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3h-memory-consolidation-only-contracts-security`
 *     in `.dependency-cruiser.cjs` extends its `from.path` regex to
 *     include `tests/architecture/fixtures/memory-consolidation/` so
 *     this file is treated as if it were under the
 *     `keiko-memory-consolidation` boundary, and its `to.path` regex
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (memory-consolidation boundary)";
