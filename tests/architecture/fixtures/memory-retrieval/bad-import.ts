 ...
 *
 * Deliberately violates the memory-retrieval strict variant of
 * ADR-0019 §"Required Dependency Direction" rule 3 (`keiko-memory-
 * retrieval` may depend only on `keiko-contracts` and
 * `keiko-security`). Exists only to prove the architecture gate fires
 * on real violations for the memory-retrieval boundary.
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3j-memory-retrieval-only-contracts-security`
 *     in `.dependency-cruiser.cjs` extends its `from.path` regex to
 *     include `tests/architecture/fixtures/memory-retrieval/` so
 *     this file is treated as if it were under the
 *     `keiko-memory-retrieval` boundary, and its `to.path` regex
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (memory-retrieval boundary)";
