 ...
 *
 * Deliberately violates the strict tools variant of ADR-0019 §"Required
 * Dependency Direction" rule 3 (`keiko-tools` may depend only on
 * `keiko-contracts`, `keiko-security`, and `keiko-workspace`). Exists only to
 * prove the architecture gate fires on real violations for the tools boundary.
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3c-tools-only-contracts-security-workspace` in
 *     `.dependency-cruiser.cjs` extends its `from.path` regex to include
 *     `tests/architecture/fixtures/tools/` so this file is treated as if it
 *     were under the `keiko-tools` boundary, and its `to.path` regex matches
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (tools boundary)";
