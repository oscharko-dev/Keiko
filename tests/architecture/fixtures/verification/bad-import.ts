 ...
 *
 * Deliberately violates the strict verification variant of ADR-0019 §"Required
 * Dependency Direction" rule 3 (`keiko-verification` may depend only on
 * `keiko-contracts`, `keiko-security`, `keiko-workspace`, and `keiko-tools`).
 * Exists only to prove the architecture gate fires on real violations for the
 ...
 *     `tests/architecture/fixtures/verification/` so this file is treated as if
 *     it were under the `keiko-verification` boundary, and its `to.path` regex
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (verification boundary)";
