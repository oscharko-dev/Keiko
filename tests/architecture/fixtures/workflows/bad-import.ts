 ...
 *
 * Deliberately violates the strict workflows variant of ADR-0019 §"Required
 * Dependency Direction" rule 5 (`keiko-workflows` may depend only on
 * `keiko-contracts`, `keiko-security`, `keiko-model-gateway`,
 * `keiko-workspace`, `keiko-tools`, `keiko-harness`, and `keiko-evidence`).
 * Exists only to prove the architecture gate fires on real violations for
 * the workflows boundary.
 *
 * Gate wiring:
 ...
 *     `tests/architecture/fixtures/workflows/` so this file is treated as if
 *     it were under the `keiko-workflows` boundary, and its `to.path` regex
 ...
 * and the strict workflows variant forbids src/cli/ in its sibling-src
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (workflows boundary)";
