 ...
 *
 * Deliberately violates the strict workspace variant of ADR-0019 §"Required
 * Dependency Direction" rule 3 (`keiko-workspace` may depend only on
 * `keiko-contracts` and `keiko-security`). Exists only to prove the architecture
 * gate fires on real violations for the workspace boundary.
 *
 * Gate wiring:
 *   - rule `adr-0019-direction-3b-workspace-only-contracts-security` in
 *     `.dependency-cruiser.cjs` extends its `from.path` regex to include
 *     `tests/architecture/fixtures/workspace/` so this file is treated as if
 *     it were under the `keiko-workspace` boundary, and its `to.path` regex
...
export const violation: string =
  typeof violationTarget === "string"
    ? violationTarget
    : "intentional ADR-0019 violation fixture (workspace boundary)";
