...
// Issue #2386 — Code Task real-authority journey. Boots the dedicated TEST-ONLY server entry
// (tests/e2e/servers/coding-runtime-2386-server.mts): the REAL buildUiHandlerDeps/createUiServer
...
// The scripted script includes the required question turn (unlike the #2385 tracer entry) so the
// browser drives the mounted question / pause / resume / follow-up routes for real. Mirrors the
// tracer harness conventions: repository paths from process.cwd(), absolute testDir at tests/e2e,
// webServer.cwd at the repository root, one deterministic chromium worker. The entry is compiled by
// the e2e-scoped tsconfig (tests/e2e/servers/tsconfig.json) because the scripted harness lives in
// *_support.ts files that are deliberately excluded from the package build.

const root = process.cwd();
const publicPort = Number(process.env.KEIKO_E2E_UI_PORT ?? String(AUTHORITY_DEFAULT_UI_PORT));
...
const serverEntry = join(
  ...
  "e2e",
  "servers",
  "coding-runtime-2386-server.mjs",
);
