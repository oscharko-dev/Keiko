...
import { CI_PORT, ciStateDir } from "../support/coding-issue-ci.js";

// Issue #3388: actual CI observation service/facade, mounted delivery approval routes, real temporary Git,
// deterministic provider boundary, and browser visual/accessibility evidence.

const root = process.cwd();
const publicPort = Number(process.env.KEIKO_E2E_UI_PORT ?? String(CI_PORT));
...
const serverEntry = join(
  ...
  "e2e",
  "servers",
  "coding-issue-ci-server.mjs",
);

export default defineConfig({
  testDir: join(root, "tests", "e2e"),
  testMatch: "coding-issue-ci.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 360_000,
