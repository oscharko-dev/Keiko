...
import { COMMIT_PORT, commitStateDir } from "../support/coding-issue-commit.js";

// Issue #3386: actual commit factory/facade, mounted review/approval routes, real temporary Git,
// deterministic provider boundary, and browser visual/accessibility evidence.

const root = process.cwd();
const publicPort = Number(process.env.KEIKO_E2E_UI_PORT ?? String(COMMIT_PORT));
...
const serverEntry = join(
  ...
  "e2e",
  "servers",
  "coding-issue-commit-server.mjs",
);

export default defineConfig({
  testDir: join(root, "tests", "e2e"),
  testMatch: "coding-issue-commit.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 360_000,
