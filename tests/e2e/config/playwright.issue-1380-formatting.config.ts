...
// per-issue editor-fidelity config (a dedicated, NON-gating coordinator-evidence config — it is not
// promoted to the `@smoke` gating set). The webServer builds the app then runs the packaged CLI UI
// against an isolated temp state dir and the e2e fixture provider config.
...
export default defineConfig({
  ...
  testMatch: "editor-formatting-1380.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 180_000,
  expect: {
