...
// PR #1907 updater regression plan: current-release notes, manual copyable commands,
// restart verification mismatch, release-link target behavior, and WindowFrame scroll clipping.

const root = process.cwd();
const publicPort = Number(process.env.KEIKO_E2E_UI_PORT ?? "32207");
const stateId = process.env.GITHUB_RUN_ID ?? `issue-1907-update-pr-${String(process.pid)}`;
...
export default defineConfig({
  ...
  testMatch: "update-pr-1907.spec.ts",
  fullyParallel: false,
  workers: 1,
  timeout: 600_000,
  expect: {
