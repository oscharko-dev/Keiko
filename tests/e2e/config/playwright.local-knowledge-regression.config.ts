...
const mockGatewayPort = Number(process.env.KEIKO_LK_E2E_MOCK_GATEWAY_PORT ?? "42186");
const mockGatewayBaseUrl = `http://127.0.0.1:${String(mockGatewayPort)}/v1`;
...
// working tree on every boot. Point that copy at the per-run loopback gateway too, so focused
// Local Knowledge runs can coexist with another checkout's fixture gateway.
...
const prepareRuntimeConfig = [
  ...
  `const config = JSON.parse(fs.readFileSync(${JSON.stringify(fixtureConfigPath)}, 'utf8'));`,
  `for (const provider of config.providers ?? []) { provider.baseUrl = ${JSON.stringify(mockGatewayBaseUrl)}; }`,
  `fs.writeFileSync(${JSON.stringify(runtimeConfigPath)}, JSON.stringify(config, null, 2));`,
].join(" ");
...
export default defineConfig({
  ...
  webServer: {
    ...
    env: {
      ...
      KEIKO_LK_E2E_MOCK_GATEWAY_PORT: String(mockGatewayPort),
