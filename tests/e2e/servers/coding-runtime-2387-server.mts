...
// the real static UI export — against a hermetic temp state dir, and injects exactly three seams:
...
// revoke), and the hermetic research transport (`researchFetchImpl`) so the governed egress port
// never touches the real network. Every route and control-plane surface — approvals, the research
