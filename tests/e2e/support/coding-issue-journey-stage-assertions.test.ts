...
describe("coding issue journey stage assertions", () => {
  ...
  // Rehearsal run-13's root cause: outcome.binding.repository carries the lowercased canonical
  // slug while remote.identity.repository carries the provider's own casing -- the two never
  // match under strict `===` for a repository like `oscharko/Wegwerf-Repo`, so a genuinely
  // completed merge and closure was reported as incomplete on a false negative.
  it("matches the provider-cased identity against the lowercased binding slug case-insensitively", () => {
    const outcome = {
      state: "completed",
      reason: "merge-and-closure-observed",
      binding: {
