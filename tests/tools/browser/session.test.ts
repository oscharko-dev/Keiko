...
describe("browser evidence manifest", () => {
  ...
  it("persists screenshots, content captures, and audit-shaped browser events", async () => {
    ...
    expect(manifest.browser.sessionId).toBe(sessionId);
    expect(manifest.browser.screenshots?.[0]).toMatchObject({
      seq: dry.seq,
      path: "browser-1.png",
      bytes: SCREENSHOT_BYTES.length,
    });
    expect(manifest.browser.screenshots?.[0]?.sha256).toHaveLength(64);
    expect(manifest.browser.contentCaptures?.[0]?.redactedHtml).toContain("secret=***");
    expect(manifest.browser.contentCaptures?.[0]?.redactedHtml).not.toContain("hunter2");
    expect(manifest.browser.events.map((event) => event.type)).toEqual(
