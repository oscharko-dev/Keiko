...
/**
 * Inline SVG of the Keiko logo. The fill is `currentColor` so the brand-green accent
 * (or any text-color class on a parent/wrapper) drives the fill without SVG edits.
 *
 * - When a `title` is provided the element gets `role="img"` and an accessible name.
 * - When `aria-hidden` is `"true"` (default) and the logo appears alongside a visible
 *   "Keiko" text label, screen readers skip the graphic entirely.
 */
export function KeikoLogo({ className, title, ...rest }: KeikoLogoProps): ReactNode {
  ...
  return (
    <svg
      ...
      className={className}
      aria-hidden={hasTitle ? undefined : ariaHidden}
