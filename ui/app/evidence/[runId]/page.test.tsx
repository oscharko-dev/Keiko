...
vi.mock("next/navigation", () => ({
  useParams: () => ({ runId: "run-detail-789" }),
...
describe("EvidenceDetailPage", () => {
  ...
  it("displays model ID and usage totals", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("5.0k")).toBeInTheDocument(); // prompt tokens
      expect(screen.getByText("1.2k")).toBeInTheDocument(); // completion tokens
    });
  });
  ...
  it("Back button is present and reachable by keyboard", async () => {
    ...
    const backBtn = screen.getByRole("button", { name: /back to evidence browser/i });
    backBtn.focus();
