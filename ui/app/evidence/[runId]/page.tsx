...
function ReasoningTrace({ entries }: { entries: EvidenceReasoningEntry[] }): ReactNode {
  return (
    <section aria-labelledby="reasoning-heading" className="mt-section">
      <h2 id="reasoning-heading" className="text-subheading text-ink">
        Reasoning trace
      </h2>
      <ol aria-label="Reasoning entries" className="mt-4 grid gap-3">
        {entries.map((entry) => (
          <li
            key={`${entry.seq}`}
            className="rounded border border-ink/10 bg-surface-subtle p-4"
          >
            ...
            {entry.rationale !== undefined && (
              <p className="mt-2 text-sm text-ink">{entry.rationale}</p>
            )}
...
export default function EvidenceDetailPage(): ReactNode {
    ...
    fetchEvidenceManifest(runId)
      ...
      .catch((err) => {
        ...
        if (err instanceof ApiError) {
          ...
          setLoadError("Failed to load evidence manifest");
        }
      });
    return () => { active = false; };
  }, [runId]);
  ...
  if (loadError !== null) {
    const safeMessage =
      errorCode === "EVIDENCE_SCHEMA"
        ? "This evidence manifest uses an unsupported schema version and cannot be displayed."
        : errorCode === "NOT_FOUND"
          ? "No evidence manifest found for this run ID."
          : loadError;

    return (
      <section aria-labelledby="evidence-detail-heading">
        <h1 id="evidence-detail-heading" className="text-heading text-ink">
          Evidence detail
        </h1>
  ...
  return (
    <section aria-labelledby="evidence-detail-heading">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span
          className={`rounded px-2 py-0.5 text-xs font-medium ${outcomeClasses(run.outcome)}`}
        >
      ...
      <section aria-labelledby="run-id-heading" className="mt-section">
        <h2 id="run-id-heading" className="text-subheading text-ink">
          Run identity
        </h2>
        <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs text-ink-muted">Task type</dt>
            <dd className="font-mono">{run.taskType}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">Fingerprint</dt>
            <dd className="font-mono text-xs">{run.fingerprint}</dd>
          </div>
          <div>
          ...
          <div>
            <dt className="text-xs text-ink-muted">Schema version</dt>
            <dd className="font-mono">v{manifest.evidenceSchemaVersion}</dd>
          </div>
        </dl>
      </section>
      ...
      <section aria-labelledby="model-heading" className="mt-section">
        <h2 id="model-heading" className="text-subheading text-ink">
          Model
        </h2>
        <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
          <div>
            <dt className="text-xs text-ink-muted">Model ID</dt>
          ...
          <div>
            ...
            <dd>
              <span
                className={`rounded px-2 py-0.5 text-xs font-medium ${costClassClasses(model.costClass)}`}
              >
                {costClassLabel(model.costClass)}
              </span>
            </dd>
          </div>
        </dl>
      ...
      <section aria-labelledby="usage-heading" className="mt-section">
        <h2 id="usage-heading" className="text-subheading text-ink">
          Usage totals
        </h2>
        <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-4">
          <div>
            <dt className="text-xs text-ink-muted">Prompt tokens</dt>
            <dd className="font-mono">{formatTokens(usageTotals.promptTokens)}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">Completion tokens</dt>
            <dd className="font-mono">{formatTokens(usageTotals.completionTokens)}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">Requests</dt>
            <dd className="font-mono">{usageTotals.requestCount.toString()}</dd>
          </div>
          <div>
            <dt className="text-xs text-ink-muted">Total latency</dt>
            <dd>{formatMs(usageTotals.totalLatencyMs)}</dd>
          </div>
        </dl>
      </section>
      ...
      {verification !== undefined && (
        <section aria-labelledby="verif-heading" className="mt-section">
          <h2 id="verif-heading" className="text-subheading text-ink">
            Verification
          </h2>
          <div className="mt-4 rounded-lg border border-ink/10 p-4">
            <div className="flex items-center gap-3">
              <span
                className={`rounded px-2 py-0.5 text-xs font-medium ${verificationStatusClasses(verification.overallStatus)}`}
              >
                ...
                {verification.results.map((r, idx) => (
                  ...
                  >
                    ...
                    {r.appliedLimits.map((lim) => (
                      ...
                      >
                        {lim.dimension}:{" "}
                        {lim.enforced ? "enforced" : "not enforced"}
                        {lim.breached === true ? " (breached)" : ""}
                      </span>
                    ))}
                  </li>
                ))}
      ...
      {patch !== undefined && (
        <section aria-labelledby="patch-meta-heading" className="mt-section">
          <h2 id="patch-meta-heading" className="text-subheading text-ink">
            Patch
          </h2>
          <dl className="mt-4 grid gap-2 text-sm sm:grid-cols-2">
            <div>
              <dt className="text-xs text-ink-muted">Proposed</dt>
              <dd>{patch.proposed ? "Yes" : "No"}</dd>
            </div>
            <div>
              <dt className="text-xs text-ink-muted">Applied</dt>
              <dd>{patch.applied ? "Yes" : "No"}</dd>
            </div>
            <div>
              <dt className="text-xs text-ink-muted">Files changed</dt>
              <dd>{patch.targetFileCount.toString()}</dd>
            </div>
          </dl>
      ...
      {failure !== undefined && (
        <section aria-labelledby="failure-heading" className="mt-section">
          <h2 id="failure-heading" className="text-subheading text-ink">
            Failure
          </h2>
      ...
      {reasoning !== undefined && reasoning.length > 0 && (
        <ReasoningTrace entries={reasoning} />
      )}
