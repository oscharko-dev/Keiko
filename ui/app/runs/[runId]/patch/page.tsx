...
// ---------------------------------------------------------------------------
// Diff viewer — accessible, non-color-only cues, no dangerouslySetInnerHTML
// ---------------------------------------------------------------------------
...
function lineClasses(kind: DiffLineKind): string {
  switch (kind) {
    case "add":
      // Green background + [+] prefix (non-color-only cue)
      return "bg-green-50 text-green-900";
    case "remove":
      // Red background + [-] prefix (non-color-only cue)
...
function DiffViewer({ diff }: DiffViewerProps): ReactNode {
  ...
  return (
    <figure aria-label="Proposed diff">
      ...
      <pre
        ...
        tabIndex={0}
        className="overflow-x-auto rounded border border-ink/10 bg-surface p-4 font-mono text-xs leading-5 focus:outline-none focus:ring-2 focus:ring-focus"
      >
        {lines.map((line, idx) => (
          // Content-derived key: line text is not guaranteed unique; use idx + kind
          <span
            key={`${String(idx)}-${line.kind}`}
            className={`block whitespace-pre ${lineClasses(line.kind)}`}
          >
            {/* Non-color-only prefix for screen readers and color-blind users */}
...
// ---------------------------------------------------------------------------
// Apply confirm dialog (inline — no modal for simplicity; focus is managed)
// ---------------------------------------------------------------------------
...
function ApplyConfirm({ onConfirm, onCancel, applying }: ApplyConfirmProps): ReactNode {
  ...
  // Move focus to confirm button when the confirmation UI appears
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <p id="apply-confirm-desc" className="mt-1 text-sm text-orange-800">
        This will write the proposed changes to your workspace. This action cannot be undone
        automatically — use version control to revert if needed.
      </p>
...
function isAppliable(report: RunReport): boolean {
  // A patch is appliable when status is dry-run-success and a diff is present
  return (
...
export default function PatchPage(): ReactNode {
  ...
  // Ref to return focus to the Apply button after dismissing confirm
  ...
  return (
    <section aria-labelledby="patch-heading">
      ...
      <section aria-labelledby="apply-heading" className="mt-section">
        ...
        {!appliable && applyState !== "done" && (
          <p className="mt-2 text-sm text-ink-muted">
            The patch can only be applied when the run is in a dry-run-success state with a valid
            proposed diff. Current status: {outcomeLabel(report.status)}.
          </p>
        )}
        ...
        {appliable && applyState === "idle" && (
          <div className="mt-4">
            <p className="text-sm text-ink-muted">
              No files have been changed yet. Applying will write the proposed diff to your
              workspace — review carefully before proceeding.
            </p>
            <button
              ref={applyBtnRef}
              type="button"
      ...
      <div className="mt-8">
        <button
          type="button"
          onClick={() => { router.push(`/runs/${encodeURIComponent(runId)}`); }}
